...
//		randomData();
	...
	public static void timeAdjust(){
		...
		ConnectFuture cf1 = connector1.connect(new InetSocketAddress("10.109.253.29", 30001));//建立连接
	...
	public static void fireMsg(){
		...
		NioSocketConnector connector1 = MinaUtil.createSocketConnector();	
		ConnectFuture cf1 = connector1.connect(new InetSocketAddress("10.109.253.31", 30001));//建立连接
		cf1.awaitUninterruptibly();//等待连接创建完成   
		
		NioSocketConnector connector2 = MinaUtil.createSocketConnector();	
		ConnectFuture cf2 = connector2.connect(new InetSocketAddress("10.109.253.13", 30001));//建立连接
		cf2.awaitUninterruptibly();//等待连接创建完成

		NioSocketConnector connector3 = MinaUtil.createSocketConnector();	
		ConnectFuture cf3 = connector3.connect(new InetSocketAddress("10.109.253.14", 30001));//建立连接
		cf3.awaitUninterruptibly();//等待连接创建完成
		
		NioSocketConnector connector4 = MinaUtil.createSocketConnector();	
		ConnectFuture cf4 = connector4.connect(new InetSocketAddress("10.109.253.18", 30001));//建立连接
		cf4.awaitUninterruptibly();//等待连接创建完成
		
/*		NioSocketConnector connector5 = MinaUtil.createSocketConnector();	
		ConnectFuture cf5 = connector5.connect(new InetSocketAddress("10.109.253.14", 30001));//建立连接
		cf5.awaitUninterruptibly();//等待连接创建完成
*/		
		/*NioSocketConnector connector6 = MinaUtil.createSocketConnector();	
		ConnectFuture cf6 = connector6.connect(new InetSocketAddress("10.109.253.26", 30001));//建立连接
		cf5.awaitUninterruptibly();//等待连接创建完成
*/				
		/*NioSocketConnector connector6 = MinaUtil.createSocketConnector();	
		ConnectFuture cf6 = connector6.connect(new InetSocketAddress("10.109.253.14", 30001));//建立连接
		cf6.awaitUninterruptibly();//等待连接创建完成
*/		
		/*NioSocketConnector connector7 = MinaUtil.createSocketConnector();	
		ConnectFuture cf7 = connector7.connect(new InetSocketAddress("10.109.253.32", 30001));//建立连接
		cf7.awaitUninterruptibly();//等待连接创建完成
*/				
/*		NioSocketConnector connector8 = MinaUtil.createSocketConnector();	
		ConnectFuture cf8 = connector8.connect(new InetSocketAddress("10.109.254.65", 30001));//建立连接
		cf8.awaitUninterruptibly();//等待连接创建完成
		*/
		try {
			Thread.sleep(1000);
		} catch (InterruptedException e) {
