...
<xsl:transform xmlns:fo="http://www.w3.org/1999/XSL/Format"
    ...
    <xsl:template match="*[contains(@class,' ui-d/screen ')]">
        ...
        <fo:block xsl:use-attribute-sets="pre">
            ...
            <xsl:if test="contains(@frame,'top')">
                <fo:block>
                    <fo:leader leader-pattern="rule" leader-length="5.65in"/>
                </fo:block>
            </xsl:if>
            <xsl:apply-templates/>
    ...
    <xsl:template
            match="*[contains(@class,' pr-d/groupcomp ')]/title|*[contains(@class,' pr-d/groupseq ')]/title|*[contains(@class,' pr-d/groupseq ')]/title"/>
    <!-- Consume title -->
