...
[contains(@class, ' topic/link ')]
    ...
    <xsl:template name="sort-links-by-role">
        ...
        <xsl:call-template name="create-links">
            <xsl:with-param name="role">cousin</xsl:with-param>
            <xsl:with-param name="type">
                <xsl:value-of select="$type"/>
            </xsl:with-param>
        </xsl:call-template>
        ...
        <xsl:call-template name="create-links">
            <xsl:with-param name="role">other</xsl:with-param>
            <xsl:with-param name="type">
                <xsl:value-of select="$type"/>
            </xsl:with-param>
        </xsl:call-template>
        ...
        <xsl:call-template name="create-links">
            <xsl:with-param name="role">#none#</xsl:with-param>
            <xsl:with-param name="type">
                <xsl:value-of select="$type"/>
            </xsl:with-param>
        </xsl:call-template>
    ...
    <xsl:template match="*" mode="determine-final-href">
        <xsl:choose>
            ...
            <xsl:when
                    test="(not(@format) and (@type='external' or @scope='external' or @scope='peer')) or (@format and not(@format='dita' or @format='DITA'))">
    ...
    <xsl:template match="*[contains(@class, ' topic/link ')][@role='child' or @role='descendant']" priority="2"
    ...
    <xsl:template match="*" mode="ditamsg:link-may-be-duplicate">
        ...
        <xsl:call-template name="output-message">
            ...
            <xsl:with-param name="msgparams">%1=<xsl:value-of select="$href"/>;%2=<xsl:value-of select="$outfile"/>
            </xsl:with-param>
        </xsl:call-template>
    </xsl:template>
