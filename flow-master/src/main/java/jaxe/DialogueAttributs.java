...
/**
 * Dialogue de modification des attributs d'un ?l?ment
 */
public class DialogueAttributs extends JDialog implements ActionListener, KeyListener, DocumentListener, ItemListener {
    ...
    public DialogueAttributs(final JFrame jframe, final JaxeDocument doc, final String titre, final Element refElement, final Element el) {
        ...
        for (int i=0; i<natt; i++) {
            ...
            obligatoire[i] = conf.attributObligatoire(refElement, att);
            ...
            if ("".equals(elval) && !obligatoire[i] && defauts[i] == null)
                attvalide = true;
            else
        ...
        for (int i=0; i<titres.length; i++) {
            ...
            if (obligatoire[i])
                label.setForeground(new Color(150, 0, 0)); // rouge fonc?
            else
                label.setForeground(new Color(0, 100, 0)); // vert fonc?
            qpane.add(label);
        }
        ...
        for (int i=0; i<champs.length; i++) {
            if (champs[i] == src) {
                ...
                if (defauts[i] == null)
                    ...
                    indexval = index;
                final boolean attvalide = !(indexval < 0 && obligatoire[i]);
                final ImageIcon icone;
                if (!attvalide)
                    icone = iconeInvalide;
                else
    ...
    public void verifChamp(final DocumentEvent e) {
        ...
        for (int i=0; i<champs.length; i++) {
            if (champs[i] instanceof JTextComponent && document == ((JTextComponent)champs[i]).getDocument()) {
                ...
                final boolean attvalide;
                final Element att = latt.get(i);
                if ("".equals(valeur) && !obligatoire[i] && defauts[i] == null)
                    attvalide = true;
                else
    ...
    public void enregistrerReponses() {
        ...
        try {
            for (int i=0; i<rep.length; i++)
                if (rep[i] != null) {
                    if (!obligatoire[i] && (("".equals(rep[i]) && defauts[i] == null) || rep[i].equals(defauts[i])))
    ...
    protected boolean checkAtt() {
        ...
        for (int i=0; i<latt.size(); i++) {
            if (obligatoire[i] && (rep[i] == null || "".equals(rep[i]))) {
                getToolkit().beep();
                if (champs[i] instanceof JTextComponent)
