...
public class WXSChoice extends WXSExplicitGroup {
    
    public WXSChoice(final Element el, final Parent parent, final WXSSchema schema) {
    ...
    public int valider(final List<WXSElement> sousElements, final int start, final boolean insertion) {
        ...
        for (int i=start; i<sousElements.size(); ) {
            ...
            for (AvecSousElements nestedParticle : nestedParticles) {
                pos = nestedParticle.valider(sousElements, i, insertion);
                if (pos > i)
                    break;
            }
            if (pos == i) {
