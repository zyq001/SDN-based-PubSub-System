...
public class JaxeFrame extends JFrame implements ComponentListener, EcouteurMAJ {
	...
	private JSplitPane splitWithTopic;
        ...
        addWindowListener(new WindowAdapter() {
            ...
            /*public void windowActivated(WindowEvent e) {
                if (System.getProperty("os.name").startsWith("Mac"))
                    ...
                    // le requestFocus provoque des activate de fen�tres en boucle sur Windows
    ...
    public void modifierSide() {
        if (afficherSide) {
            ...
            splitWithTopic = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT);
            //将主题树列表加在此处
//            split.
//            split.setLeftComponent(TopicTreeManager.LibTreeForSchema);
            split.setLeftComponent(paneScrollPane);
            split.setRightComponent(sidepane);
            split.setDividerLocation(360);//分割线的位置
            splitWithTopic.setLeftComponent(TopicTreeManager.LibTreeForSchema);
            splitWithTopic.setRightComponent(split);
            splitWithTopic.setDividerLocation(120);//分割线的位置

            //split.addPropertyChangeListener(new EcouteurPropriete());
            getContentPane().add(splitWithTopic, BorderLayout.CENTER);
        } else {
            sidepane = null;
            arbrexml = null;
            allowed = null;
...
//        getContentPane().setVisible(true);
        ...
        attente.dispose();
//        getContentPane().getComponent(1).remremove(0);
        getContentPane().validate();
