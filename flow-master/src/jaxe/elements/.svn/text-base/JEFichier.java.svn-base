...
public class JEFichier extends JaxeElement {
        ...
        if (urlimg == null || (fimg != null && (!fimg.exists() || !fimg.isFile()))) {
            ...
            if (liste != null && liste.hasNext()) {
                ...
                try {
                    ...
                    if (img == null)
                        erreur = true;
                } catch (Exception ex) { // parfois ImageIO.read lance une java.awt.color.CMMException
    ...
    public static void collerImage(final Image img, final JaxeDocument doc, final Position pos, final Element refElement) {
        ...
        final String srcAttr = conf.valeurParametreElement(refElement, "srcAtt", defaultSrcAttr);
        final ArrayList<Element> attributs = conf.listeAttributs(refElement);
        for (final Element attdef : attributs) {
            if (srcAttr.equals(conf.nomAttribut(attdef)))
                newel.setAttributeNS(conf.espaceAttribut(attdef), srcAttr, dossier.getName() + File.separator + nouveauNom);
        }
    ...
    @Override
    public void effacer() {
        if (label != null && label.getIcon() instanceof ImageIcon && ((ImageIcon)label.getIcon()).getImage() != null) {
            // pour libérer la mémoire immédiatement
            final Image img = ((ImageIcon)label.getIcon()).getImage();
            label.setIcon(null);
            img.flush();
        }
        super.effacer();
