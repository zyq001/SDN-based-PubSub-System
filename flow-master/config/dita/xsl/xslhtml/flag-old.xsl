...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
            ...
            <xsl:when test="contains($props,',')">
                ...
                <xsl:if test="not($propsValue='')">
                    <xsl:call-template name="output-message">
                        <xsl:with-param name="msgnum">042</xsl:with-param>
                        <xsl:with-param name="msgsev">I</xsl:with-param>
                        <xsl:with-param name="msgparams">%1=<xsl:value-of select="$propName"/>
                        </xsl:with-param>
                    </xsl:call-template>
                </xsl:if>
    ...
    <xsl:template name="ext-mark-prop">
        ...
        <xsl:choose>
            <xsl:when xmlns:exsl="http://exslt.org/common" test="exsl:node-set($flag-result)/img">
        ...
        <xsl:choose> <!-- Ensure there's an image to get, otherwise don't insert anything -->
            <xsl:when test="$FILTERDOC/val/prop[@att=$flag-att][@val=$firstflag][@action='flag'][@img]">
                ...
                <xsl:variable name="imgsrc"
                              select="$FILTERDOC/val/prop[@att=$flag-att][@val=$firstflag][@action='flag']/@img"/>

                <img>
                    <xsl:attribute name="src">
                        <xsl:if test="string-length($PATH2PROJ) > 0">
                    ...
                    <xsl:attribute
                            name="alt"> <!-- always insert an ALT - if it's blank, assume the user didn't want to fill it. -->
                        <xsl:value-of
                                select="$FILTERDOC/val/prop[@att=$flag-att][@val=$firstflag][@action='flag']/@alt"/>
                    </xsl:attribute>
                </img>
            </xsl:when>
            <xsl:otherwise/>
    ...
    <xsl:template name="revblock-deprecated">
        <xsl:choose>
            <xsl:when
                    test="@rev and not($FILTERFILE='') and ($DRAFT='yes')"> <!-- draft rev mode, add div w/ rev attr value -->
    ...
    <xsl:template name="revstyle-deprecated">
        ...
        <xsl:choose>
            ...
            <xsl:otherwise>
                <xsl:apply-templates/>
                <!-- no active rev color - just apply templates -->
            </xsl:otherwise>
        </xsl:choose>
    </xsl:template>
