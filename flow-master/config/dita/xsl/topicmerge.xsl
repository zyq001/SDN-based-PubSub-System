...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
    ...
    <xsl:template match="*[contains(@class,' map/topicref ')][@href][not(@href='')][not(@print='no')]">
        <xsl:variable name="topicrefClass">
            <xsl:value-of select="@class"/>
        </xsl:variable>
        <xsl:comment>Start of imbed for
        ...
        <xsl:choose>
            ...
            <xsl:when test="contains(@href,'#')">
                ...
                <xsl:variable name="targetName">
                    <xsl:value-of
                            select="name(document($sourcefile,/)//*[@id=$sourcetopic][contains(@class,' topic/topic ')][1])"/>
                </xsl:variable>
                <xsl:if test="$targetName and not($targetName='')">
                    <xsl:element name="{$targetName}">
                        <xsl:apply-templates
                                select="document($sourcefile,/)//*[@id=$sourcetopic][contains(@class,' topic/topic ')][1]/@*"
                                mode="copy-element"/>
    ...
    <xsl:template match="@href" mode="copy-element" priority="1">
        ...
        <xsl:choose>
            ...
            <xsl:when
                    test="(parent::*[contains(@class,' topic/xref ')] or parent::*[contains(@class,' topic/link ')]) and (not(../@format) or ../@format='dita' or ../@format='DITA')">
                <xsl:choose>
                        ...
                        <xsl:choose>
                            <xsl:when test="contains($refer-path,'/')">
                                ...
                                <xsl:variable name="href-value">
                                    <xsl:value-of
                                            select="generate-id(//*[contains(@class,' topic/topic ')][@id=$topic-id]//*[@id=$target-id]/@id)"/>
                                </xsl:variable>
                                <xsl:if test="not($href-value='')">
                                    <xsl:attribute name="href">
                                        <xsl:text>#</xsl:text><xsl:value-of select="$href-value"/>
                                    </xsl:attribute>
                                </xsl:if>
                            </xsl:when>
                            <xsl:otherwise>
                                <xsl:variable name="href-value">
                                    <xsl:value-of
                                            select="generate-id(//*[contains(@class,' topic/topic ')][@id=$refer-path]/@id)"/>
                                </xsl:variable>
                                <xsl:if test="not($href-value='')">
                                    <xsl:attribute name="href">
                                        <xsl:text>#</xsl:text><xsl:value-of select="$href-value"/>
                        ...
                        <xsl:if test="$file-name-doc and not($file-name-doc='')">
                            <xsl:choose>
                                <xsl:when test="contains($refer-path,'/')">
                                    ...
                                    <xsl:variable name="href-value">
                                        <xsl:value-of
                                                select="generate-id($file-name-doc//*[contains(@class,' topic/topic ')][@id=$topic-id]//*[@id=$target-id]/@id)"/>
                                    </xsl:variable>
                                    <xsl:if test="not($href-value='')">
                                        <xsl:attribute name="href">
                                            <xsl:text>#</xsl:text><xsl:value-of select="$href-value"/>
                                ...
                                <xsl:otherwise>
                                    <xsl:variable name="href-value">
                                        <xsl:value-of
                                                select="generate-id($file-name-doc//*[contains(@class,' topic/topic ')][@id=$refer-path]/@id)"/>
                                    </xsl:variable>
                                    <xsl:if test="not($href-value='')">
                                        <xsl:attribute name="href">
                                            <xsl:text>#</xsl:text><xsl:value-of select="$href-value"/>
                                ...
                                <xsl:when test="$current-doc//*[contains(@class,' topic/topic ')]/@id">
                                    <xsl:attribute name="href">
                                        <xsl:text>#</xsl:text><xsl:value-of
                                            select="generate-id($current-doc//*[contains(@class,' topic/topic ')][1]/@id)"/>
                                    </xsl:attribute>
                                </xsl:when>
                                <xsl:otherwise>
                                    <xsl:text>#</xsl:text><xsl:value-of
                                        select="generate-id($current-doc//*[contains(@class,' topic/topic ')][1])"/>
                                </xsl:otherwise>
                            </xsl:choose>
                        </xsl:if>
                    </xsl:otherwise>
