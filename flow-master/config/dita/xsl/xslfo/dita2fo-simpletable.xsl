...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
                ...
                <xsl:if test="../../@relcolwidth and not(../../@relcolwidth='')">
                    ...
                    <xsl:attribute name="column-width">proportional-column-width(
                        <xsl:call-template name="get-proportional-width">
                            ...
                            <xsl:with-param name="all-widths">
    ...
    <xsl:template name="get-proportional-width">
        ...
        <xsl:choose>
            ...
            <xsl:otherwise>
                <xsl:call-template name="get-proportional-width">
                    <xsl:with-param name="entry-num">
                        <xsl:value-of select="$entry-num - 1"/>
                    </xsl:with-param>
                    <xsl:with-param name="all-widths">
                        <xsl:value-of select="substring-after($all-widths,' ')"/>
                    </xsl:with-param>
                </xsl:call-template>
            </xsl:otherwise>
        </xsl:choose>
    ...
    <xsl:template match="*[contains(@class,' topic/stentry ')]">
        ...
        <xsl:variable name="thisrow">
            <xsl:number level="single" count="strow"/>
        </xsl:variable>
            ...
            <xsl:when test="$thisnum=$localkeycol">
                <fo:table-cell start-indent="2pt" background-color="#fafafa" padding="2pt"
                    ...
                    <fo:block font-weight="bold">
                        ...
                        <xsl:apply-templates/>
                        <!-- don't use "apply-for-phrases" for editor -->
                    </fo:block>
                </fo:table-cell>
            </xsl:when>
            <xsl:otherwise>
