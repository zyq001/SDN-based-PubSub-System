...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="@id">
        ...
        <fo:block>
            <xsl:attribute name="id">
                ...
                <xsl:value-of select="."/>
                <!-- generate id according to current node -->
            </xsl:attribute>
    ...
    <!-- turned off for now; not needed for authoring -->
    ...
    <xsl:template name="setscale">
        <xsl:if test="@scale">
            ...
            <xsl:attribute name="font-size"><xsl:value-of select="@scale div 10"/>pt
            </xsl:attribute>
        </xsl:if>
    </xsl:template>
