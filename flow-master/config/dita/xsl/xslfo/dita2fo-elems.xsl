...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="*[contains(@class,' topic/xshortdesc ')]">
        <fo:block start-indent="{$basic-start-indent}" background-color="#F0C0F0">
            <fo:block background-color="red">The &lt;shortdesc&gt; element is also reflected following the prolog.
            </fo:block>
            <xsl:apply-templates/>
        </fo:block>
    </xsl:template>
    ...
    <xsl:template match="*[contains(@class,' topic/image ')]">
        ...
        <xsl:if test="$ARTLBL='yes'">
            <fo:block font-weight="bold">[<xsl:value-of select="$IP"/> <xsl:value-of select="@href"/>]
            </fo:block>
        </xsl:if>
    </xsl:template>
    ...
    <xsl:template name="insert-graphic">
        <fo:external-graphic> <!-- apply scaling attributes here? -->
            <xsl:attribute name="src">url(<xsl:call-template name="get-image-uri"/>)
            </xsl:attribute>
            <xsl:choose>
                <xsl:when test="@scale">
                ...
                <xsl:otherwise><!--xsl:attribute name="content-width">scale-to-fit</xsl:attribute--></xsl:otherwise>
                <!-- no effect -->
            </xsl:choose>
        </fo:external-graphic>
    </xsl:template>
    ...
    <xsl:template name="topic-image">
        ...
        <xsl:element name="img">
            ...
            <xsl:choose>
                ...
                <xsl:otherwise>
                    <xsl:attribute name="alt">
                        <xsl:value-of select="*[contains(@class,' topic/textalt')]"/>
                    </xsl:attribute>
                </xsl:otherwise>
            </xsl:choose>
        </xsl:element>
    ...
    <xsl:template match="*[contains(@class,' topic/required-cleanup ')]">
        <xsl:if test="$DRAFT='yes'">
            <fo:inline background="yellow" color="#CC3333">  <!-- indents won't apply here; not a block context -->
                ...
                <xsl:attribute name="border-width">thin</xsl:attribute>
                <!-- set id -->
                <fo:inline font-weight="bold">Required Cleanup
                    <xsl:if test="string(@remap)">(<xsl:value-of select="@remap"/>)
                    </xsl:if>
                    <xsl:text>: </xsl:text>
                </fo:inline>
                <xsl:apply-templates/>
