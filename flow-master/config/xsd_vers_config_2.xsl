...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema"
    ...
    <xsl:variable name="nom-fichier-xsd">
        <xsl:call-template name="nom-fichier-uri">
            <xsl:with-param name="uri" select="$jaxe-uri-xml"/>
        </xsl:call-template>
    </xsl:variable>
    ...
    <xsl:template match="xs:element" mode="affichage">
        ...
        <xsl:choose>
            <xsl:when
                    test="parent::xs:schema and not(@substitutionGroup) and count(//xs:element[@ref=$nom or substring-after(@ref,':')=$nom])=0">
                <AFFICHAGE_ELEMENT element="{$nom}" type="division">
                    <xsl:call-template name="titre-affichage"/>
                </AFFICHAGE_ELEMENT>
            </xsl:when>
            <xsl:when
                    test="xs:complexType and not(xs:complexType/@mixed='true') and not(.//xs:element|.//xs:group|.//xs:extension)">
