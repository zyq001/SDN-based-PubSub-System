...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="commentaires">
        <xsl:choose>
            <xsl:when test="/EXPOSE/METADONNEES/OPTIONS/COMMENTAIRES">
                <xsl:value-of select="/EXPOSE/METADONNEES/OPTIONS/COMMENTAIRES"/>
            </xsl:when>
    ...
    <xsl:param name="impression">
        <xsl:choose>
            <xsl:when test="/EXPOSE/METADONNEES/OPTIONS/IMPRESSION">
                <xsl:value-of select="/EXPOSE/METADONNEES/OPTIONS/IMPRESSION"/>
            </xsl:when>
            <xsl:otherwise>non</xsl:otherwise>
        </xsl:choose>
    </xsl:param>
    ...
    <xsl:template match="EXPOSE">
        ...
        <html>
            <head>
                ...
                <link rel="stylesheet" type="text/css" href="config_exposeV3/jaxe_expose_transitions.css">
                    <xsl:text> </xsl:text>
                </link>
                <link rel="stylesheet" type="text/css" href="config_exposeV3/jaxe_expose.css">
                    <xsl:text> </xsl:text>
                </link>
                <link rel="timesheet" type="application/smil+xml" href="config_exposeV3/jaxe_expose.smil">
                    <xsl:text> </xsl:text>
                </link>


                <script type="text/javascript" src="config_exposeV3/timesheets.js">
                    <xsl:text> </xsl:text>
                </script>
                <script type="text/javascript" src="config_exposeV3/timesheets-navigation.js">
                    <xsl:text> </xsl:text>
                </script>
    ...
    <xsl:template name="css">
        ...
        h5 { text-align: center; font-size: 90%; font-weight: bold; font-family: serif; margin-bottom: 0.5em; color:
        #963232 }
        h6 { text-align: center; font-size: 70%; font-weight: bold; font-family: serif; margin-bottom: 0.5em; color:
        #963232 }
        ...
        .slide { page-break-after: always; background: #FFFFEB; margin: 2em 0.5em; padding: 0.5em;
        <xsl:if test="$impression='non'">border: gray outset</xsl:if>
        }
        ...
        <div class="slide" id="slide{$numero}">
            ...
            <table class="bas" width="100%">
                <tr>
                    ...
                    <td align="center" class="tiers">
                        <tt><xsl:value-of select="$numero"/>/<xsl:value-of select="count(//SECTION)"/>
                        </tt>
                    </td>
                    <td align="right" class="tiers">
                        <tt>
                        ...
                        <xsl:choose>
                            ...
                            <xsl:when test="normalize-space(@type)!=''">
                                ...
                                <ul type="{@type}">
                                    ...
                                    <xsl:apply-templates select="EL"/>
                                    <li class="signeplus">
                                        <img src="config_exposeV3/signe_plus.jpg"/>
                                    </li>
                                </ul>
    ...
    <xsl:template match="EL">
        ...
        <li id="{$id_el}">
            ...
            <xsl:if test="count(descendant::LISTE)!=0">
                <xsl:apply-templates select="child::*[name()!='EMPHASE']"/>
            </xsl:if>
