        ...
        <LANGAGE>
            <xsl:choose>
                ...
                <xsl:otherwise>
                    <SCHEMA_SIMPLE>
                        <xsl:for-each
                                select="RACINE/BALISE[count(../../MENU//BALISE[@nom=current()/@nom])=0] | MENU//BALISE">
                            <xsl:variable name="texte">
                                <xsl:choose>
                                    <xsl:when test="count(TEXTE) &gt; 0">autorise</xsl:when>
                                    <xsl:when
                                            test="count(//ENSEMBLE[@nom=current()/SOUSBALISE/@ensemble]/TEXTE) &gt; 0">
                                        autorise
                                    </xsl:when>
