...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xalanredirect="http://xml.apache.org/xalan/redirect"
    ...
    <xsl:variable name="chemin-xsl">
        <xsl:call-template name="rep-fichier">
            <xsl:with-param name="chemin" select="$jaxe-fichier-xsl"/>
        </xsl:call-template>
    </xsl:variable>
            ...
            <xsl:for-each select="//XPAGES">
                ...
                <xsl:if test="$label-fichiers!='' and not(preceding-sibling::XPAGES/@labelfichiers=$label-fichiers)">
                    <xsl:variable name="res"
                                  select="faire-images:copierDossierContrib(string(concat($chemin-contrib, $sep, $label-fichiers)), string($chemin-site))"/>
                    <xsl:if test="$res!='ok'">
                        <xsl:message>Erreur à la copie du dossier contrib
                            <xsl:value-of select="$label-fichiers"/>
                        </xsl:message>
                    </xsl:if>
                </xsl:if>
            </xsl:for-each>
            ...
            <xsl:call-template name="ecrire-fichier">
                ...
                <xsl:with-param name="contenu">
                    <IMAGES><xsl:text>
                        ...
                        <xsl:for-each
                                select="//FICHIER[@nom!='']|//LOGO[@fichier!='' and not(starts-with(@fichier,'http://'))]">
                            ...
                            <xsl:variable name="localisation">
                                <xsl:choose>
                                    <xsl:when test="ancestor::ENVIMAGE[1]/@localisation!=''">
                                        <xsl:value-of select="ancestor::ENVIMAGE[1]/@localisation"/>
                                    </xsl:when>
                                    <xsl:otherwise>texte</xsl:otherwise>
                                </xsl:choose>
                            </xsl:variable>
                            <xsl:variable name="dossierfichier"
                                          select="concat($chemin-contrib, $sep, $label-fichiers)"/>
                            <xsl:variable name="dossier-res" select="concat($chemin-site, '/pages_', $label-fichiers)"/>
                            <xsl:variable name="res"
                                          select="faire-images:transfo(string($dossierfichier), string(@nom|@fichier), string($localisation), string($dossier-res))"/>
                            <xsl:if test="faire-images:resok($res)='erreur'">
                                <xsl:message>Erreur à la réduction de l'image
                                    <xsl:value-of select="@nom|@fichier"/> de la contrib
                                    <xsl:value-of select="$label-fichiers"/>
                                </xsl:message>
                            </xsl:if>
                            ...
                            <IMAGE nom="{@nom|@fichier}" contrib="{$label-fichiers}" localisation="{$localisation}"
                                   redim="{$redim}" largeur1="{faire-images:largeur1($res)}"
                                   hauteur1="{faire-images:hauteur1($res)}">
                                <xsl:if test="$redim='oui'">
                                    <xsl:attribute name="largeur2">
                                        <xsl:value-of select="faire-images:largeur2($res)"/>
                                    </xsl:attribute>
                                    <xsl:attribute name="hauteur2">
                                        <xsl:value-of select="faire-images:hauteur2($res)"/>
                                    </xsl:attribute>
                                </xsl:if>
                            ...
                            <xsl:if test="$localisation!='texte'">
                                <xsl:variable name="res2"
                                              select="faire-images:transfo(string($dossierfichier), string(@nom|@fichier), string('texte'), string($dossier-res))"/>
                                <xsl:if test="faire-images:resok($res2)='erreur'">
                                    <xsl:message>Erreur à la réduction pour l'impression de l'image
                                        <xsl:value-of select="@nom|@fichier"/> de la contrib
                                        <xsl:value-of select="$label-fichiers"/>
                                    </xsl:message>
                                </xsl:if>
                                <xsl:variable name="redim2">
                                    <xsl:choose>
                                ...
                                <IMAGE nom="{@nom|@fichier}" contrib="{$label-fichiers}" localisation="texte"
                                       redim="{$redim2}" largeur1="{faire-images:largeur1($res2)}"
                                       hauteur1="{faire-images:hauteur1($res2)}">
                                    <xsl:if test="$redim2='oui'">
                                        <xsl:attribute name="largeur2">
                                            <xsl:value-of select="faire-images:largeur2($res2)"/>
                                        </xsl:attribute>
                                        <xsl:attribute name="hauteur2">
                                            <xsl:value-of select="faire-images:hauteur2($res2)"/>
                                        </xsl:attribute>
                                    </xsl:if>
    ...
    <xsl:template name="faire-equations">
        <xsl:for-each select="//EQUATION | //EQUATEX">
            ...
            <xsl:variable name="dossier-res"
                          select="concat($chemin-site, '/pages_', ancestor::XPAGES[1]/@labelfichiers)"/>
            <xsl:variable name="res"
                          select="faire-images:creerImageEquation(string(.), string($numero), string($label-ensemble), string($dossier-res))"/>
            <xsl:if test="$res!='ok'">
                <xsl:message>Erreur à la copie du dossier contrib
                    <xsl:value-of select="$label-ensemble"/>
                </xsl:message>
            </xsl:if>
        </xsl:for-each>
    </xsl:template>
