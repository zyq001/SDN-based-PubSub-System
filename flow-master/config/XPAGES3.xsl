...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xalanredirect="http://xml.apache.org/xalan/redirect"
                ...
                version="1.0" extension-element-prefixes="xalanredirect exsl">

    <xsl:output method="html" indent="yes" encoding="UTF-8"/>
    <xsl:strip-space elements="CODE"/>
    <!-- pour NBSP (sinon un retour à la ligne est ajouté après le template) -->
    ...
    <xsl:param name="sortie">public</xsl:param>
    <!-- public|production|tuteurs -->
    ...
    <xsl:variable name="chemin-xml">
        <xsl:call-template name="rep-fichier">
            <xsl:with-param name="chemin" select="$jaxe-fichier-xml"/>
        </xsl:call-template>
    </xsl:variable>
    <xsl:variable name="uri-chemin-xml">
        <xsl:call-template name="rep-uri">
            <xsl:with-param name="chemin" select="$jaxe-uri-xml"/>
        </xsl:call-template>
    </xsl:variable>

    <!-- URI du fichier XSL -->
    <xsl:variable name="uri-xsl">
        <xsl:call-template name="rep-uri">
            <xsl:with-param name="chemin" select="$jaxe-uri-xsl"/>
        </xsl:call-template>
    </xsl:variable>

    <!-- chemin du répertoire dans lequel les fichiers HTML seront générés -->
    <xsl:param name="chemin-site">
        <xsl:choose>
            <xsl:when test="$jaxe-fichier-destination!=''">
                <xsl:call-template name="rep-fichier">
                    <xsl:with-param name="chemin" select="$jaxe-fichier-destination"/>
                </xsl:call-template>
            </xsl:when>
            <xsl:otherwise>
                <xsl:value-of select="$chemin-xml"/>
            </xsl:otherwise>
        </xsl:choose>
        <xsl:value-of select="concat($sep, 'site')"/>
    </xsl:param>
    <xsl:param name="uri-site">
        <xsl:choose>
            <xsl:when test="$jaxe-uri-destination!=''">
                <xsl:call-template name="rep-uri">
                    <xsl:with-param name="chemin" select="$jaxe-uri-destination"/>
                </xsl:call-template>
            </xsl:when>
            <xsl:otherwise>
                <xsl:value-of select="$uri-chemin-xml"/>
            </xsl:otherwise>
        </xsl:choose>
        /site
    </xsl:param>

    <xsl:param name="rubriques">oui</xsl:param>
    <!-- oui/non -->

    <!-- chemin du répertoire contribXML (dans lequel se trouvent toutes les contributions) -->
    <xsl:variable name="chemin-contrib">
        <xsl:call-template name="rep-fichier">
    ...
    <xsl:variable name="langue">
        <xsl:choose>
            <xsl:when test="/XPAGES/INFORMATIONS/LANGUE">
                <xsl:value-of select="/XPAGES/INFORMATIONS/LANGUE"/>
            </xsl:when>
    ...
    <xsl:variable name="roles"
                  select="'|introduction|conclusion|prerequis|objectifs|rappel|definition|demonstration|exemple|conseil|remarque|attention|complement|methode|activite|exercice|bibliographie|'"/>


    <xsl:template match="/">

        <!-- nom du fichier XML (qui doit être égal au label de l'ensemble) -->
        <xsl:variable name="nomfichierxml">
            <xsl:call-template name="nom-fichier">
                <xsl:with-param name="chemin" select="$jaxe-fichier-xml"/>
            </xsl:call-template>
        </xsl:variable>

        <!-- nom du répertoire du fichier XML (qui doit être égal au label de l'ensemble) -->
        <xsl:variable name="nomrep">
            <xsl:call-template name="nom-fichier">
                <xsl:with-param name="chemin" select="$chemin-xml"/>
            </xsl:call-template>
        </xsl:variable>
        ...
        <html>
            <head>
                <title>
                    <xsl:value-of select="$messages/message[@label='Aperçu']"/>
                </title>
            </head>
            <body>
                ...
                <xsl:if test="$nomrep = XPAGES/INFORMATIONS/LABEL and $nomfichierxml = concat($nomrep, '.xml')">
                    ...
                    <p>
                        <a href="site/index.html">
                            <xsl:value-of select="$messages/message[@label='aller à la page d_accueil du site']"/>
                        </a>
                    </p>
                    <p>
                        <a href="site/pages_{XPAGES/INFORMATIONS/LABEL}/sommaire.html">
                            <xsl:value-of select="$messages/message[@label='aller au sommaire']"/>
                        </a>
                    </p>
                    <p>
                        <a href="site/pages_{XPAGES/INFORMATIONS/LABEL}/impression.html">
                            <xsl:value-of select="$messages/message[@label='aller à la page d_impression']"/>
                        </a>
                    </p>
                </xsl:if>
    ...
    <xsl:template name="infos-erreurs">
        ...
        <xsl:choose>
            <xsl:when test="count(/XPAGES/INFORMATIONS) = 0">
                <p>
                    <b>
                        <xsl:value-of select="$messages/message[@label='Attention !']"/>
                    </b>
                    &#xA0;<xsl:value-of
                        select="$messages/message[@label='L_élément INFORMATIONS est obligatoire sous la racine XPAGE. C_est un élément à insérer en premier dans le document, et qui permet de spécifier son label et son titre.']"/>
                </p>
            </xsl:when>
            ...
            <xsl:when test="count(/XPAGES/INFORMATIONS/LABEL) = 0">
                <p>
                    ...
                    &#xA0;<xsl:value-of
                        select="$messages/message[@label='Le label est obligatoire dans les informations. Il permet d_identifier le document, et de faire des liens vers ce document depuis d_autres documents XPAGES. Ce label doit correspondre au nom du fichier XML et au nom du dossier parent.']"/>
                </p>
            </xsl:when>
            ...
            <xsl:when test="$nomrep != /XPAGES/INFORMATIONS/LABEL or $nomfichierxml != concat($nomrep, '.xml')">
                <p>
                    ...
                    &#xA0;<xsl:value-of
                        select="$messages/message[@label='Le nom du dossier doit être égal au label du document, et le nom du fichier XML doit être le label avec l_extension .xml.']"/>
                </p>
                <p><xsl:value-of select="$messages/message[@label='Actuellement, le nom du dossier est']"/>&#xA0;
                    <tt>
                        <xsl:value-of select="$nomrep"/>
                    </tt>
                    <xsl:value-of select="$messages/message[@label=', le label du document est']"/>&#xA0;
                    <tt>
                        <xsl:value-of select="/XPAGES/INFORMATIONS/LABEL"/>
                    </tt>
                    <xsl:value-of select="$messages/message[@label=', et le nom du fichier est']"/>&#xA0;
                    <tt>
                        <xsl:value-of select="$nomfichierxml"/>
                    </tt>
                </p>
            </xsl:when>
        </xsl:choose>
    ...
    <xsl:template match="XPAGES">
        ...
        <xsl:variable name="interface">interface/
            <xsl:choose>
                <xsl:when test="count(ancestor-or-self::XPAGES[INFORMATIONS/INTERFACE!='']) &gt; 0">
                    <xsl:value-of
                            select="ancestor-or-self::XPAGES[INFORMATIONS/INTERFACE!=''][1]/INFORMATIONS/INTERFACE"/>
                </xsl:when>
                <xsl:otherwise>
                    <xsl:value-of select="$interface_par_defaut"/>
                </xsl:otherwise>
            </xsl:choose>
        </xsl:variable>

        <xsl:variable name="nom-fichier-index">
            <xsl:choose>
                <xsl:when test="@contribution='oui'">index.html</xsl:when>
                <xsl:otherwise><xsl:value-of select="INFORMATIONS/LABEL"/>_index.html
                </xsl:otherwise>
            </xsl:choose>
        </xsl:variable>
        <xsl:if test="@contribution!='oui' or count(PAGE[@label='index'])=0">
            <xsl:call-template name="ecrire-fichier">
                <xsl:with-param name="fichier"
                                select="concat($chemin-site, $sep, 'pages_', @labelfichiers, $sep, $nom-fichier-index)"/>
                <xsl:with-param name="contenu">
        ...
        <xsl:if test="count(ancestor::XPAGES)=0">
            <xsl:call-template name="ecrire-fichier">
                <xsl:with-param name="fichier"
                                select="concat($chemin-site, $sep, 'pages_', INFORMATIONS/LABEL, $sep, 'sommaire.html')"/>
                <xsl:with-param name="contenu">
            ...
            <xsl:if test="//GLOSSAIRE">
                <xsl:call-template name="ecrire-fichier">
                    <xsl:with-param name="fichier"
                                    select="concat($chemin-site, $sep, 'pages_', INFORMATIONS/LABEL, $sep, 'glossaire.html')"/>
                    <xsl:with-param name="contenu">
        ...
        <xsl:for-each select="PAGE//ENVIMAGE">
            ...
            <xsl:variable name="nomsfichiers">|
                <xsl:for-each select=".//FICHIER">
                    <xsl:value-of select="concat(@nom, '|')"/>
                </xsl:for-each>
            </xsl:variable>
            <xsl:variable name="localisation" select="@localisation"/>
            <xsl:variable name="redim"
                          select="$images/IMAGE[@contrib=$label-fichiers and contains($nomsfichiers, concat('|',@nom,'|')) and @localisation=$localisation][1]/@redim"/>
        ...
        <xsl:variable name="nom-fichier-impression">
            <xsl:choose>
                <xsl:when test="@contribution='oui'">impression.html</xsl:when>
                <xsl:otherwise><xsl:value-of select="INFORMATIONS/LABEL"/>_impression.html
                </xsl:otherwise>
            </xsl:choose>
        </xsl:variable>
        <xsl:call-template name="ecrire-fichier">
            <xsl:with-param name="fichier"
                            select="concat($chemin-site, $sep, 'pages_', @labelfichiers, $sep, $nom-fichier-impression)"/>
            <xsl:with-param name="contenu">
    ...
    <xsl:template name="zone_bandeau">
        ...
        <div class="titre_site_1">
            <xsl:value-of select="ancestor-or-self::XPAGES[$nb]/INFORMATIONS/TITRE"/>
        </div>
                            ...
                            <xsl:otherwise>
                                <td class="rubrique">
                                    ...
                                    <xsl:for-each select="./descendant::PAGE[1]">
                                        <a href="{concat($prefixe,'pages_', ancestor::XPAGES[1]/@labelfichiers, '/', @label,'.html')}">
    ...
    <xsl:template name="page-index">
        ...
        <html>
            <head>
                ...
                <xsl:variable name="nb" select="count(ancestor-or-self::XPAGES)"/>
                <link rel="Contents"
                      href="{$prefixe}pages_{ancestor-or-self::XPAGES[$nb]/INFORMATIONS/LABEL}/sommaire.html"/>
                ...
                <li>
                    <xsl:choose>
                        <xsl:when test="self::PAGE">
                            <a href="{@label}.html" class="lien">
                                <xsl:choose>
                                    <xsl:when test="@titre!=''">
                                        <xsl:value-of select="@titre"/>
                                    </xsl:when>
                                    <xsl:otherwise>
                                        <xsl:value-of select="$messages/message[@label='page']"/>
                        ...
                        <xsl:when test="self::XPAGES">
                            <xsl:choose>
                                <xsl:when test="*[self::PAGE|self::XPAGES][1][self::PAGE]">
                                    <a href="{$prefixe}pages_{@labelfichiers}/{PAGE[1]/@label}.html" class="lien">
                                        <xsl:value-of select="INFORMATIONS/TITRE"/>
                                    </a>
                                </xsl:when>
                                <xsl:otherwise>
                                    ...
                                    <a href="{$prefixe}pages_{ancestor-or-self::XPAGES[1]/@labelfichiers}/{$nom-fichier-index}"
                                       class="lien">
                                        <xsl:value-of select="INFORMATIONS/TITRE"/>
                                    </a>
                                </xsl:otherwise>
    ...
    <xsl:template match="PAGE">
        ...
        <xsl:call-template name="ecrire-fichier">
            <xsl:with-param name="fichier"
                            select="concat($chemin-site, $sep, 'pages_', $label-contrib, $sep, @label,'.html')"/>
            <xsl:with-param name="contenu">
    ...
    <xsl:template name="contenu-page">
        ...
        <html>
            <head>
                ...
                <xsl:variable name="nom-fichier-index-parent">
                    <xsl:choose>
                        <xsl:when test="../@contribution='oui'">index.html</xsl:when>
                        <xsl:otherwise><xsl:value-of select="../INFORMATIONS/LABEL"/>_index.html
                        </xsl:otherwise>
                ...
                <xsl:if test=".//ENVIMAGE[@localisation='texte' or @localisation='page']">
                    <xsl:variable name="formats">|
                        <xsl:for-each select=".//ENVIMAGE[@localisation='texte' or @localisation='page']//FICHIER">
    ...
    <xsl:template name="zone_logo">
        ...
        <div class="zone_logo">
            <xsl:for-each select="ancestor-or-self::XPAGES/INFORMATIONS/LOGO[1]">
                <xsl:variable name="prefsrcimg">
                    <xsl:if test="not(starts-with(@fichier,'http://'))">
                        <xsl:value-of select="concat($prefixe, 'pages_', ../LABEL, '/')"/>
                    </xsl:if>
                </xsl:variable>
                <xsl:choose>
                    <xsl:when test="@lien!=''">
                        <a href="{@lien}">
                            <img src="{$prefsrcimg}{@fichier}" alt="{@fichier}" border="0"/>
                        </a>
                    </xsl:when>
                    <xsl:otherwise>
                        <img src="{$prefsrcimg}{@fichier}" alt="{@fichier}"/>
                    </xsl:otherwise>
                </xsl:choose>
            </xsl:for-each>
        </div>
    ...
    <xsl:template name="zone_sommaire">
        <xsl:param name="interface">interface/<xsl:value-of select="$interface_par_defaut"/>
        </xsl:param>
        <xsl:param name="prefixe">../</xsl:param>
                ...
                <table style="width: 100%">
                    <tr>
                        ...
                        <td style="text-align: center">
                            <a href="{$prefixe}index.html" title="{$messages/message[@label='Entrée du site']}">
                                <img src="{$prefixe}{$interface}/home.png" width="25" height="25"
                                     alt="{$messages/message[@label='Entrée du site']}" border="0"/>
                            </a>
                        </td>
                        <td style="text-align: center">
                            <a href="{$prefixe}pages_{ancestor-or-self::XPAGES[$nb]/INFORMATIONS/LABEL}/sommaire.html"
                               title="{$messages/message[@label='Sommaire']}">
                                <img src="{$prefixe}{$interface}/sommaire_petit.png" width="25" height="25" border="0"
                                     alt="{$messages/message[@label='Sommaire']}"/>
                            </a>
                        </td>
                        <xsl:if test="//GLOSSAIRE">
                            <td style="text-align: center">
                                <a href="{$prefixe}pages_{ancestor-or-self::XPAGES[$nb]/INFORMATIONS/LABEL}/glossaire.html"
                                   title="{$messages/message[@label='Glossaire']}">
                                    <img src="{$prefixe}{$interface}/glossaire_petit.png" width="25" height="25"
                                         border="0" alt="{$messages/message[@label='Glossaire']}"/>
                                </a>
                            </td>
                        </xsl:if>
                        <td style="text-align: center">
                            <xsl:for-each select="ancestor-or-self::XPAGES[1]">
                                ...
                                <a href="{$prefixe}pages_{@labelfichiers}/{$nom-fichier-impression}"
                                   title="{INFORMATIONS/TITRE} : {$messages/message[@label='Page pour l_impression']}"
                                   target="_blank">
                                    <img src="{$prefixe}{$interface}/impression.png" width="25" height="25" border="0"
                                         alt="{$messages/message[@label='Page pour l_impression']}"/>
                                </a>
                            </xsl:for-each>
                        </td>
                        <td style="text-align: center">
                            <xsl:for-each
                                    select="preceding::PAGE[1][@label!='index' or count(ancestor::XPAGES) &gt; 1]">
                                <a href="{$prefixe}pages_{ancestor::XPAGES[1]/@labelfichiers}/{@label}.html"
                                   accesskey="j" title="{$messages/message[@label='Page précédente']}">
                                    <img src="{$prefixe}{$interface}/petiteflechegauche.png" width="25" height="16"
                                         alt="&lt;-" border="0"/>
                                </a>
                            </xsl:for-each>
                        </td>
                        <td style="text-align: center">
                            <xsl:for-each
                                    select="./descendant::PAGE[1] | following::PAGE[current()/self::PAGE and position()=1]">
                                <a href="{$prefixe}pages_{ancestor::XPAGES[1]/@labelfichiers}/{@label}.html"
                                   accesskey="l" title="{$messages/message[@label='Page suivante']}">
                                    <img src="{$prefixe}{$interface}/petiteflechedroite.png" width="25" height="16"
                                         alt="-&gt;" border="0"/>
                                </a>
                            </xsl:for-each>
                        </td>
                    ...
                    <td>
                        <h1>
                            <xsl:if test="self::PAGE and @role != '' and contains($roles, concat('|', @role, '|'))">
                                <img src="{$prefixe}{$interface}/icones_sections/{@role}.png" width="40" height="40"
                                     alt="{@role}" class="icone_page"/>
                            </xsl:if>
                            <xsl:call-template name="titre-page"/>
                        </h1>
                    </td>
    ...
    <xsl:template name="contenu-body">
        ...
        <xsl:variable name="rub"
                      select="$rubriques='oui' and (not(ancestor-or-self::XPAGES[INFORMATIONS/RUBRIQUES]) or ancestor-or-self::XPAGES[INFORMATIONS/RUBRIQUES!=''][1]/INFORMATIONS/RUBRIQUES='oui')"/>
                    ...
                    <tr>
                        <td>
                            <xsl:for-each
                                    select="preceding::PAGE[1][@label!='index' or count(ancestor::XPAGES) &gt; 1]">
                                <a href="{$prefixe}pages_{ancestor::XPAGES[1]/@labelfichiers}/{@label}.html"
                                   title="{$messages/message[@label='Page précédente']}">
                                    <img src="{$prefixe}{$interface}/flechegauche.png" width="48" height="31"
                                         alt="{$messages/message[@label='Page précédente']}" border="0"/>
                                </a>
                            </xsl:for-each>
                        </td>
                        <td style="text-align: right">
                            <xsl:for-each
                                    select="./descendant::PAGE[1] | following::PAGE[current()/self::PAGE and position()=1]">
                                <a href="{$prefixe}pages_{ancestor::XPAGES[1]/@labelfichiers}/{@label}.html"
                                   title="{$messages/message[@label='Page suivante']}">
                                    <img src="{$prefixe}{$interface}/flechedroite.png" width="48" height="31"
                                         alt="{$messages/message[@label='Page suivante']}" border="0"/>
                                </a>
                            </xsl:for-each>
                        </td>
                    </tr>
    ...
    <xsl:template name="contenu-impression">
        ...
        <html>
            <head>
                <title>
                    <xsl:value-of
                            select="concat(INFORMATIONS/TITRE, ' : ', $messages/message[@label='Page pour l_impression'])"/>
                </title>
                <link type="text/css" rel="stylesheet" href="{$prefixe}{$interface}/{$feuille-de-style}"/>
            </head>
            <body>
                        ...
                        <xsl:otherwise>
                            ...
                            <xsl:if test="position()!=last()">
                                ...
                                <xsl:if test="count(following-sibling::PAGE)=0">
                                    <div style="page-break-after: always"/>
                                    <!-- pour les navigateurs comprenant CSS 2 -->
                                </xsl:if>
                            </xsl:if>
                        </xsl:otherwise>
                    </xsl:choose>
                ...
                <xsl:if test=".//QCM">
                    ...
                    <xsl:for-each select=".//QCM">
                        <p style="font-family: monospace">pages_<xsl:value-of
                                select="ancestor::XPAGES[1]/INFORMATIONS/LABEL"/>/<xsl:value-of
                                select="ancestor::PAGE/@label"/>.html
                        </p>
                        <h3>QCM
                            <xsl:if test="@titre!=''">
                            ...
                            <xsl:for-each select="QUESTIONQCM">
                                <li>Question
                                    ...
                                    <xsl:for-each select="REPONSEQCM[@bonne='oui']">
                                        <i>Solution :</i>
                                        réponse<xsl:number from="QCM" count="REPONSEQCM"/>)
                ...
                <xsl:if test=".//EXERCICE[@type='auto-évaluation'] or (.//EXERCICE and ($sortie='production' or $sortie='tuteurs')) or .//EXERCICE/QUESTION/AIDE">
                    ...
                    <xsl:for-each
                            select=".//EXERCICE[count(QUESTION/AIDE) &gt; 0 or ((@type='auto-évaluation' or $sortie='production' or $sortie='tuteurs') and count(QUESTION/SOLUTION) &gt; 0)]">
        ...
        <xsl:choose>
            ...
            <xsl:when test="$pos &gt; 1">
                ...
                <xsl:for-each select="ancestor-or-self::XPAGES[position()=$pos]">
                    <xsl:choose>
                        <xsl:when test="*[self::PAGE|self::XPAGES][1][self::PAGE]">
                            <a href="{$prefixe}pages_{@labelfichiers}/{PAGE[1]/@label}.html" class="sommaire2">&#xA0;<xsl:value-of
                                    select="INFORMATIONS/TITRE"/>
                            </a>
                        </xsl:when>
                        <xsl:otherwise>
                            <xsl:variable name="nom-fichier-index">
                            ...
                            <a href="{$prefixe}pages_{@labelfichiers}/{$nom-fichier-index}" class="sommaire2">&#xA0;<xsl:value-of
                                    select="INFORMATIONS/TITRE"/>
                            </a>
                        </xsl:otherwise>
                    </xsl:choose>
            ...
            <xsl:otherwise>
                <div class="fondsel1sommaire">
                    <xsl:for-each select="ancestor::XPAGES[2]">
                        <xsl:choose>
                            <xsl:when test="*[self::PAGE|self::XPAGES][1][self::PAGE]">
                                <a href="{$prefixe}pages_{@labelfichiers}/{PAGE[1]/@label}.html">
                                    <img height="11" width="11" alt="-" border="0"
                                         src="{$prefixe}{$interface}/listeactive.png"/>
                                </a>
                            </xsl:when>
                            <xsl:otherwise>
                                <xsl:variable name="nom-fichier-index">
                                ...
                                <a href="{$prefixe}pages_{@labelfichiers}/{$nom-fichier-index}">
                                    <img height="11" width="11" alt="-" border="0"
                                         src="{$prefixe}{$interface}/listeactive.png"/>
                                </a>
                            </xsl:otherwise>
                    ...
                    <div class="divsommaire">
                        ...
                        <xsl:for-each select="../*[(self::PAGE and @label!='index') or self::XPAGES]">
                            <xsl:call-template name="element-sommaire">
                                <xsl:with-param name="labelpage">
                                    <xsl:value-of select="$labelpage"/>
                                </xsl:with-param>
                                <xsl:with-param name="interface">
                                    <xsl:value-of select="$interface"/>
                                </xsl:with-param>
    ...
    <xsl:template name="element-sommaire">
        ...
        <xsl:choose>
            ...
            <xsl:when test="self::XPAGES">
                <div class="fondsel1sommaire">
                    <img src="{$prefixe}{$interface}/listeinactive.png" border="0" alt="+" width="11" height="11"/>
                    <xsl:choose>
                        <xsl:when test="*[self::PAGE|self::XPAGES][1][self::PAGE]">
                            <a href="{$prefixe}pages_{@labelfichiers}/{PAGE[1]/@label}.html" class="sommaire3">&#xA0;<xsl:value-of
                                    select="INFORMATIONS/TITRE"/>
                            </a>
                        </xsl:when>
                        <xsl:otherwise>
                            <a href="{$prefixe}pages_{INFORMATIONS/LABEL}/index.html" class="sommaire3">&#xA0;<xsl:value-of
                                    select="INFORMATIONS/TITRE"/>
                            </a>
                        </xsl:otherwise>
                    </xsl:choose>
            ...
            <xsl:otherwise>
                <div class="fondsel1sommaire">
                    <a href="{$prefixe}pages_{ancestor::XPAGES[1]/@labelfichiers}/{@label}.html" class="sommairepage">
                        &#x2022;
                        <xsl:value-of select="@titre"/>
                    </a>
                </div>
    ...
    <xsl:template name="sommaire-page">
        ...
        <div class="sommaire">
            <xsl:call-template name="sous-sommaire">
                <xsl:with-param name="pos">
                    <xsl:value-of select="count(ancestor::XPAGES)"/>
                </xsl:with-param>
                <xsl:with-param name="interface">
                    <xsl:value-of select="$interface"/>
                </xsl:with-param>
    ...
    <xsl:template match="DATECRE">
        <xsl:value-of select="$messages/message[@label='Date de création&#xA0;:']"/>
        <i>
            <xsl:apply-templates/>
        </i>
    </xsl:template>
    ...
    <xsl:template match="DATEMAJ">
        <xsl:value-of select="$messages/message[@label='Date de mise à jour&#xA0;:']"/>
        <i>
            <xsl:apply-templates/>
        </i>
    </xsl:template>
    ...
    <xsl:template match="SECTION">
        ...
        <xsl:variable name="classe">
            <xsl:choose>
                <xsl:when
                        test="(@role='introduction' or @role='conclusion' or @role='exercice') and @importance='grande'">
    ...
    <xsl:template match="TD">
        ...
        <xsl:variable name="trpos">
            <xsl:number from="TABLEAU" count="TR"/>
        </xsl:variable>
        ...
        <xsl:choose>
            <xsl:when test="$localisation='icône' and $affichage='web'">
                <xsl:for-each select=".//FICHIER[1]">
                    ...
                    <xsl:variable name="fichierhtml"
                                  select="concat($prefixe,'pages_', $label-fichiers, '/', 'html_images/envimage', $numero, '.html')"/>
                           ...
                           onclick="window.open('{$fichierhtml}', '', 'scrollbars=yes,scrolling=auto,toolbar=no,directories=no,menubar=no,status=no,width={$largeur_fenetre},height={@hauteur1+200},left=' + (100+(typeof window.screenX != 'undefined' ? window.screenX : window.screenLeft)) + ',top=' + (100+(typeof window.screenY != 'undefined' ? window.screenY : window.screenTop))); return false">
                            ...
                            <xsl:choose>
                                <xsl:when test="$format='PNG' or $format='GIF' or $format='JPEG'">
                                    <img src="{$prefixe}pages_{$label-fichiers}/images_icone/{$nomfichier}"
                                         width="{@largeur2}" height="{@hauteur2}" alt="{$nomfichier}"
                                         style="float: right" class="iconeimage"/>
                                </xsl:when>
                                <xsl:when test="$format='MPEG' or $format='MPEG4' or $format='OGG' or $format='WEBM'">
                                    <img src="{$prefixe}{$interface}/video.png" width="50" height="50" alt="vidéo"
                                         style="float: right" border="0"/>
                                </xsl:when>
                                <xsl:otherwise>
                                    <xsl:value-of
                                            select="$messages/message[@label='format inconnu (vérifier l_extension du nom du fichier)']"/>
                                </xsl:otherwise>
                            </xsl:choose>
                        </a>
                    </xsl:for-each>
                        ...
                        <a href="#" onclick="imagette({position()}); return false;">
                            ...
                            <xsl:choose>
                                <xsl:when test="position()=1">
                                    <img src="{$prefixe}{$interface}/imagesel.png" width="59" height="52"
                                         alt="image {position()}" border="0"/>
                                </xsl:when>
                                <xsl:otherwise>
                                    <img src="{$prefixe}{$interface}/image.png" width="59" height="52"
                                         alt="image {position()}" border="0"/>
                                </xsl:otherwise>
                            </xsl:choose>
                        </a>
    ...
    <xsl:template name="largeur-fichier">
        ...
        <xsl:variable name="nomfichier" select="@nom"/>
        <xsl:for-each
                select="$images/IMAGE[@contrib=$label-fichiers and @nom=$nomfichier and @localisation='texte'][1]">
            <xsl:choose>
                <xsl:when test="$affichage='impression' and @anim='oui' and @largeur1*2 &gt;= 700">700</xsl:when>
                <xsl:when test="$affichage='impression' and @anim='oui' and @largeur1*2 &lt; 700">
                    <xsl:value-of select="@largeur1*2"/>
                </xsl:when>
                <xsl:when test="@redim='oui'">
                    <xsl:value-of select="@largeur2"/>
                </xsl:when>
                <xsl:otherwise>
                    <xsl:value-of select="@largeur1"/>
                </xsl:otherwise>
            </xsl:choose>
        </xsl:for-each>
    </xsl:template>
    ...
    <xsl:template name="largeur-max">
        ...
        <xsl:choose>
            <xsl:when test="following-sibling::FICHIER">
                ...
                <xsl:choose>
                    <xsl:when test="$largeur-ce-fichier &gt; $max-suivants">
                        <xsl:value-of select="$largeur-ce-fichier"/>
                    </xsl:when>
                    <xsl:otherwise>
                        <xsl:value-of select="$max-suivants"/>
                    </xsl:otherwise>
                </xsl:choose>
            </xsl:when>
            <xsl:otherwise>
    ...
    <xsl:template name="contenu-envimage">
        ...
        <div class="{$classe-div}">
            <xsl:if test="@localisation='page' and $affichage='web'">
                ...
                <xsl:attribute name="id">figure<xsl:value-of select="$pos"/>
                </xsl:attribute>
                <xsl:if test="$pos &gt; 1">
            ...
            <xsl:if test="$affichage='impression' or @localisation='texte'">
                ...
                <xsl:variable name="largeur-div">
                    <xsl:choose>
                        <xsl:when test="$largeur-max &gt;= 500">
                            <xsl:value-of select="$largeur-max + 20"/>
                        </xsl:when>
                        <xsl:otherwise>520</xsl:otherwise>
                    </xsl:choose>
                </xsl:variable>
                <xsl:attribute name="style">width: <xsl:value-of select="$largeur-div"/>px
                </xsl:attribute>
            </xsl:if>
            <xsl:if test="@titre!=''">
                <div class="titrefigure">
                    <xsl:value-of select="@titre"/>
                </div>
            </xsl:if>
    ...
    <xsl:template name="html-figure">
        ...
        <xsl:call-template name="ecrire-fichier">
            <xsl:with-param name="fichier"
                            select="concat($chemin-site, $sep, 'pages_', $label-fichiers, $sep, 'html_images', $sep, 'envimage', $numero, '.html')"/>
            <xsl:with-param name="contenu">
                <xsl:text disable-output-escaping='yes'>&lt;!DOCTYPE html>
</xsl:text>
                <html>
                            ...
                            <xsl:for-each select="FICHIER|FORMATS">
                                <xsl:choose>
                                    <xsl:when test="self::FICHIER">
                                        ...
                                        <xsl:for-each
                                                select="$images/IMAGE[@contrib=$label-fichiers and @nom=$cheminfichier and @localisation=$localisation][1]">
                                                ...
                                                <xsl:for-each select="FICHIER[1]">
                                                    ...
                                                    <xsl:for-each
                                                        ...
                                                        <xsl:call-template name="html-fichier">
                                                            ...
                                                            <xsl:with-param name="prefixe" select="$prefixe"/>
                                                            <xsl:with-param name="interface" select="$interface"/>
                                                            <xsl:with-param name="label-fichiers"
                                                                            select="$label-fichiers"/>
                                                            <xsl:with-param name="cheminfichier"
    ...
    <xsl:template name="script-video">
        ...
        <script type="text/javascript"
                src="{$htmlversvideojs}?player={$htmlversplayer}&amp;codebase={$prefixe}&amp;archive={$codebaseverscortado}"></script>
    </xsl:template>
    ...
    <xsl:template name="html-fichier">
        ...
        <xsl:variable name="chemindepuishtml"
                      select="concat($prefixe, 'pages_', $label-fichiers, '/', $cheminfichier)"/>
        ...
        <xsl:choose>
            ...
            <xsl:when test="$format='MPEG' and $affichage='web'">
                <embed src="{$chemindepuishtml}" width="{$largeur}" height="{$hauteur}">
                    <noembed>
                        <xsl:value-of select="$messages/message[@label='Erreur: ce navigateur ne gère pas EMBED']"/>
                    </noembed>
                </embed>
            </xsl:when>
            ...
            <xsl:when
                    test="($format='MPEG' or $format='MPEG4' or $format='OGG' or $format='WEBM') and $affichage!='web'">
                <p>
                    <img src="{$prefixe}{$interface}/video.png" width="50" height="50" alt="vidéo {$format}"
                         border="0"/>
                </p>
            </xsl:when>
            <xsl:otherwise>
    ...
    <xsl:template match="FORMATS">
        ...
        <xsl:variable name="largeur"
                      select="$images/IMAGE[@contrib=$label-fichiers and @nom=$nomfichier1 and @localisation=$localisation][1]/@largeur1"/>
        <xsl:variable name="hauteur"
                      select="$images/IMAGE[@contrib=$label-fichiers and @nom=$nomfichier1 and @localisation=$localisation][1]/@hauteur1"/>
    ...
    <xsl:template match="CREDIT">
        ...
        <div class="credit">
            <em>
                <xsl:value-of select="$messages/message[@label='Crédit :']"/>
            </em>
            <xsl:apply-templates>
                <xsl:with-param name="prefixe" select="$prefixe"/>
            </xsl:apply-templates>
        ...
        <xsl:choose>
            <xsl:when test="$affichage='impression'">
                <p>
                    ...
                    <img src="{$prefixe}{$interface}/{$icone}" width="50" height="50" border="0" alt="{$icone}"/>
                </p>
            </xsl:when>
            <xsl:when test="@localisation='icône'">
                ...
                <xsl:variable name="numero">
                    <xsl:number level="any" from="XPAGES[@contribution='oui']" count="APPLICATION|APPLET|FLASH"/>
                </xsl:variable>
                <xsl:variable name="nomfichiermedia"><xsl:value-of select="concat('media',$numero)"/>.html
                </xsl:variable>
                <xsl:variable name="fichiermedia">
                    <xsl:value-of select="concat($rep,$sep,$nomfichiermedia)"/>
                </xsl:variable>
                <xsl:variable name="largeur_fenetre">
                    <xsl:choose>
                        <xsl:when test="@largeur &lt; 450">500</xsl:when>
                        <xsl:otherwise>
                            <xsl:value-of select="@largeur + 50"/>
                        </xsl:otherwise>
                    </xsl:choose>
                </xsl:variable>
                <xsl:variable name="hauteur_fenetre">
                    <xsl:choose>
                        <xsl:when test="LEGENDE">
                            <xsl:value-of select="@hauteur + 200"/>
                        </xsl:when>
                        <xsl:otherwise>
                            <xsl:value-of select="@hauteur + 100"/>
                        </xsl:otherwise>
                    </xsl:choose>
                </xsl:variable>
                            ...
                            <head>
                                ...
                                <title>
                                    <xsl:choose>
                                        ...
                                        <xsl:otherwise>
                                            <xsl:value-of select="concat('application ',$numero)"/>
                                        </xsl:otherwise>
                                    </xsl:choose>
                                </title>
                                <link type="text/css" rel="stylesheet"
                                      href="{$prefixe}{$interface}/{$feuille-de-style}"/>
                            </head>
                            <body>
    ...
    <xsl:template match="APPLET|FLASH">
        ...
        <xsl:choose>
            <xsl:when test="$affichage='impression'">
                ...
                <p>
                    <img src="{$prefixe}{$interface}/{$icone}" width="45" height="45" border="0" alt="{$icone}"/>
                </p>
            </xsl:when>
            <xsl:when test="@localisation='icône'">
                <xsl:variable name="label-fichiers" select="ancestor::XPAGES[1]/@labelfichiers"/>
                ...
                <xsl:call-template name="ecrire-fichier">
                    ...
                    <xsl:with-param name="contenu">
                        ...
                        <html>
                            <head>
                                <title>
                                    <xsl:value-of select="concat('applet ',$numero)"/>
                                </title>
                                <link type="text/css" rel="stylesheet"
                                      href="{$prefixe}{$interface}/{$feuille-de-style}"/>
                            </head>
    ...
    <xsl:template name="contenu-media">
        ...
        <div class="{$classe-div}">
            <xsl:if test="@localisation!='page'">
                <xsl:attribute name="style">width: <xsl:value-of select="@largeur + 12"/>px;
                </xsl:attribute>
            </xsl:if>
            <xsl:if test="@titre!=''">
                <div class="titrefigure">
            ...
            <xsl:choose>
                <xsl:when test="self::APPLICATION and @type='applet'">
                    <xsl:variable name="fichier-application">
                        <xsl:choose>
                            <xsl:when test="@contribution!=''">
                                <xsl:value-of select="concat('pages_', @contribution, '/', @fichier)"/>
                            </xsl:when>
                            <xsl:otherwise>
                    ...
                    <applet code="{PARAM[@nom='code']/@valeur}" archive="{$fichier-application}" width="{@largeur}"
                            height="{@hauteur}">
                ...
                <xsl:when test="self::APPLICATION and @type='flash'">
                    <xsl:variable name="fichier-application">
                        <xsl:choose>
                            <xsl:when test="@contribution!=''">
                                <xsl:value-of select="concat($prefixe, 'pages_', @contribution, '/', @fichier)"/>
                            </xsl:when>
                            <xsl:otherwise>
                                <xsl:value-of select="@fichier"/>
                            </xsl:otherwise>
                    ...
                    <object type="application/x-shockwave-flash" data="{$fichier-application}" width="{@largeur}"
                            height="{@hauteur}" title="{@titre}">
                ...
                <xsl:when test="self::APPLICATION and @type='html'">
                    ...
                    <xsl:variable name="chemin-fichiers">
                        <xsl:call-template name="chemin-inverse">
                            <xsl:with-param name="chemin">
                                <xsl:value-of select="@fichier"/>
                            </xsl:with-param>
                        </xsl:call-template>
                        <xsl:if test="@contribution!=''">../pages_<xsl:value-of
                                select="ancestor::XPAGES[1]/@labelfichiers"/>/
                        </xsl:if>
                    </xsl:variable>
                    <xsl:variable name="lienhtml">
                        <xsl:value-of select="concat($fichier-application, '?chemin_fichiers=', $chemin-fichiers)"/>
                        <xsl:for-each select="PARAM">&amp;<xsl:value-of select="concat(@nom, '=', @valeur)"/>
                        </xsl:for-each>
                    </xsl:variable>
                    <iframe src="{$lienhtml}" width="{@largeur}" height="{@hauteur}" style="border:none"
                            frameborder="0"/>
                </xsl:when>
                ...
                <xsl:when test="self::FLASH">
                    <object type="application/x-shockwave-flash" data="{@fichier}" width="{@largeur}"
                            height="{@hauteur}" title="{@titre}">
                        <param name="movie" value="{@fichier}"/>
                        <xsl:variable name="parametres">
                            <xsl:for-each select="PARAM">
    ...
    <xsl:template match="EXERCICE">
        ...
        <div class="exercice">
            ...
            <xsl:element name="h{count(ancestor::SECTION)+2}">
                <img src="{$prefixe}{$interface}/icones_sections/exercice.png" width="40" height="40" alt="exercice"
                     class="icone_section"/>
                <xsl:choose>
                    <xsl:when test="@titre!=''">
                        <xsl:value-of select="@titre"/>
                    ...
                    <xsl:otherwise>
                        <xsl:value-of select="$messages/message[@label='Exercice']"/>
                    </xsl:otherwise>
                </xsl:choose>
            </xsl:element>
    ...
    <xsl:template match="QUESTION">
        <xsl:param name="prefixe">../</xsl:param>
        <xsl:param name="affichage">web</xsl:param>
        <xsl:variable name="numquestion">
            <xsl:number level="any" from="EXERCICE" count="QUESTION"/>
        </xsl:variable>
        <div class="question">
            <b><xsl:value-of select="concat($messages/message[@label='Question'], ' ', $numquestion)"/>)
            </b>
            <xsl:apply-templates select="ENONCE">
                <xsl:with-param name="prefixe" select="$prefixe"/>
                <xsl:with-param name="affichage" select="$affichage"/>
    ...
    <xsl:template match="AIDE | SOLUTION" mode="afficher-div">
        ...
        <div id="{$idaide}" style="display:none" class="contenu-aide-solution">
            <xsl:apply-templates/>
        </div>
    </xsl:template>
    ...
    <xsl:template match="AIDE | SOLUTION" mode="lien">
        ...
        <a id="{$idlien}" href="{$nomFichier}" target="_blank" class="{$classelien}"
           onclick="afficher_masquer('{$idlien}'); return false;">
            <xsl:value-of select="$messages/message[@label=$msglabel]"/>
        </a>
    </xsl:template>
    ...
    <xsl:template match="POINTS">
        [
        <em>
            <xsl:value-of select="concat(., ' ', $messages/message[@label='points'])"/>
        </em>
        ]
    </xsl:template>
    ...
    <xsl:template match="QCM">
        ...
        <div class="qcm">
            <xsl:call-template name="auteurs-exo"/>
            <xsl:element name="h{count(ancestor::SECTION)+2}">
                <img src="{$prefixe}{$interface}/icones_sections/qcm.png" width="40" height="40" alt="qcm"
                     class="icone_section"/>
                <xsl:choose>
                    <xsl:when test="@titre!=''">
                        <xsl:value-of select="@titre"/>
                    ...
                    <xsl:otherwise>
                        <xsl:value-of select="$messages/message[@label='QCM']"/>
                    </xsl:otherwise>
                </xsl:choose>
            </xsl:element>
            <xsl:apply-templates select="ENONCE"/>
            ...
            <xsl:choose>
                <xsl:when test="$affichage='web'">
                    <xsl:variable name="numqcm">
                        <xsl:number level="any" from="PAGE" count="QCM"/>
                    </xsl:variable>
    ...
    <xsl:template match="ENONCEQCM">
        <xsl:variable name="numquestion">
            <xsl:number level="any" from="QCM" count="QUESTIONQCM"/>
        </xsl:variable>
        <!--<div class="enonceqcm">-->
    ...
    <xsl:template match="REPONSEQCM">
        ...
        <xsl:variable name="numrep">
            <xsl:number level="any" from="QUESTIONQCM" count="REPONSEQCM"/>
        </xsl:variable>
        ...
        <xsl:variable name="name">
            <xsl:choose>
                <xsl:when test="$type='radio'">
                    <xsl:value-of select="concat('r',$numquestion)"/>
                </xsl:when>
                <xsl:otherwise>
                    <xsl:value-of select="concat('r',$numquestion,'_',$numrep)"/>
                </xsl:otherwise>
            </xsl:choose>
        </xsl:variable>
        ...
        <xsl:variable name="url">
            <xsl:choose>
                <xsl:when test="@contribution='' or not(@contribution)">
                    ...
                    <xsl:value-of
                            select="concat($prefixe, 'pages_', ancestor::XPAGES[1]/@labelfichiers, '/', @page, '.html')"/>
                </xsl:when>
                <xsl:when test="@page='' or not(@page)">
                    <xsl:for-each select="//XPAGES[INFORMATIONS/LABEL=current()/@contribution][1]/descendant::PAGE[1]">
                        <xsl:value-of
                                select="concat($prefixe,'pages_', ancestor::XPAGES[1]/@labelfichiers, '/', @label,'.html')"/>
                    </xsl:for-each>
                </xsl:when>
                <xsl:otherwise>
                    <xsl:value-of
                            select="concat($prefixe, 'pages_', //XPAGES[INFORMATIONS/LABEL=current()/@contribution][1]/@labelfichiers, '/', @page, '.html')"/>
                </xsl:otherwise>
            </xsl:choose>
        </xsl:variable>
        <a href="{$url}" class="lien">
            <xsl:apply-templates>
                <xsl:with-param name="prefixe" select="$prefixe"/>
            </xsl:apply-templates>
        </a>
    ...
    <xsl:template match="LIENWEB">
        <xsl:param name="prefixe">../</xsl:param>
        <a href="{@url}" class="lien" target="_blank">
            <xsl:apply-templates>
                <xsl:with-param name="prefixe" select="$prefixe"/>
            </xsl:apply-templates>
        </a>
                ...
                <xsl:choose>
                    <xsl:when test="$localisation='icône'">
                        <xsl:variable name="nom"
                                      select="ancestor::PAGE[1]//ENVIMAGE[@label=$label][1]//FICHIER[1]/@nom"/>
                        <xsl:variable name="lien"
                                      select="concat('../pages_', ancestor::XPAGES[1]/@labelfichiers, '/', 'html_images/envimage', $numero, '.html')"/>
                        <xsl:variable name="label-fichiers" select="ancestor::XPAGES[1]/@labelfichiers"/>
                        <xsl:variable name="largeur1"
                                      select="$images/IMAGE[@contrib=$label-fichiers and @nom=$nom]/@largeur1"/>
                        <xsl:variable name="hauteur1"
                                      select="$images/IMAGE[@contrib=$label-fichiers and @nom=$nom]/@hauteur1"/>
                        <xsl:variable name="largeur_fenetre">
                            <xsl:choose>
                                <xsl:when test="$largeur1 &lt; 460">500</xsl:when>
                                <xsl:otherwise>
                        ...
                        <a href="{$lien}" class="lienimage" target="_blank"
                           onclick="window.open('{$lien}', '', 'scrollbars=yes,scrolling=auto,toolbar=no,directories=no,menubar=no,status=no,width={$largeur_fenetre},height={$hauteur1+200},left=' + (100+(typeof window.screenX != 'undefined' ? window.screenX : window.screenLeft)) + ',top=' + (100+(typeof window.screenY != 'undefined' ? window.screenY : window.screenTop))); return false">
                            <xsl:apply-templates/>
                        </a>
                    </xsl:when>
                    <xsl:when
                            test="$localisation='page' and count(ancestor::PAGE[1]//ENVIMAGE[@localisation='page'])=1">
                        <xsl:variable name="nom"
                                      select="ancestor::PAGE[1]//ENVIMAGE[@label=$label][1]//FICHIER[1]/@nom"/>
                        <xsl:variable name="label-fichiers" select="ancestor::XPAGES[1]/@labelfichiers"/>
                        <xsl:variable name="redim"
                                      select="$images/IMAGE[@contrib=$label-fichiers and @nom=$nom and @localisation=$localisation][1]/@redim"/>
                        <xsl:choose>
                            <xsl:when test="$redim='oui'">
                                <xsl:variable name="lien"
                                              select="concat('../pages_', ancestor::XPAGES[1]/@labelfichiers, '/', 'html_images/envimage', $numero, '.html')"/>
                    ...
                    <xsl:when
                        ...
                        <a href="#" onclick="imagette({$position}); return false;" class="lienimage">
                            <xsl:apply-templates/>
                        </a>
                    </xsl:when>
                    <xsl:otherwise>
    ...
    <xsl:template match="SYMBOLE">
        ...
        <xsl:choose>
            <xsl:when test="@nom != ''">
                <xsl:variable name="nomlettre">
                    <xsl:call-template name="nom-fichier">
                        <xsl:with-param name="chemin" select="@nom"/>
                    </xsl:call-template>
                </xsl:variable>
    ...
    <xsl:template match="EQUATION|EQUATEX">
        ...
        -->
        <xsl:variable name="numero">
            <xsl:number from="XPAGES" count="EQUATION|EQUATEX" level="any"/>
        </xsl:variable>
        <img alt="{@texte}" src="{$prefixe}pages_{$label-fichiers}/equations_{$label-ensemble}/equation{$numero}.png"
             class="maths"/>
    </xsl:template>
        ...
        <xsl:variable name="definition">
            <xsl:choose>
                <xsl:when test="normalize-space(.)!=''">
                    <xsl:apply-templates select="." mode="texte-simple"/>
                </xsl:when>
                <xsl:otherwise>
                    <xsl:apply-templates select="//GLOSSAIRE[@mot=$motref or @ref=$motref][normalize-space(.)!=''][1]"
                                         mode="texte-simple"/>
                </xsl:otherwise>
            </xsl:choose>
        </xsl:variable>
        <xsl:variable name="motreftrans" select="translate($motref,' àâéèêîïôöùû','_aaeeeiioouu')"/>
        <a href="{$prefixe}pages_{ancestor::XPAGES[last()]/INFORMATIONS/LABEL}/glossaire.html#{$motreftrans}"
           title="{$definition}" class="glossaire" target="_blank">
            <xsl:value-of select="@mot"/>
        </a>
    </xsl:template>

    <xsl:template match="GLOSSAIRE" mode="texte-simple">
        <xsl:apply-templates mode="texte-simple"/>
    </xsl:template>

    <xsl:template match="EM" mode="texte-simple">*<xsl:apply-templates mode="texte-simple"/>*
    </xsl:template>

    <xsl:template match="SUP" mode="texte-simple">^(<xsl:value-of select="."/>)
    </xsl:template>

    <xsl:template match="SUB" mode="texte-simple">_(<xsl:value-of select="."/>)
    </xsl:template>

    <xsl:template match="CODE" mode="texte-simple">
        <xsl:apply-templates mode="texte-simple"/>
    </xsl:template>

    <xsl:template match="NBSP" mode="texte-simple">
        <xsl:text> </xsl:text>
    </xsl:template>
    ...
    <xsl:template match="BIBLIOGRAPHIE">
        ...
        <xsl:element name="h{count(ancestor::SECTION)+2}">
            <img src="{$prefixe}{$interface}/icones_sections/bibliographie.png" width="40" height="40"
                 alt="bibliographie" class="icone_section"/>
            <xsl:choose>
                <xsl:when test="@titre!=''">
                    <xsl:value-of select="@titre"/>
                </xsl:when>
                <xsl:otherwise>
                    <xsl:value-of select="$messages/message[@label='Bibliographie']"/>
                </xsl:otherwise>
            </xsl:choose>
        </xsl:element>
        <ul>
    ...
    <xsl:template match="REFARTICLE">
        <li>
            ...
            .
            <i>
                <xsl:value-of select="@titrePeriodique"/>
            </i>
            ,
    ...
    <xsl:template match="CODE">
        <xsl:choose>
            <xsl:when
                    test="parent::PAGE or parent::SECTION or ((parent::ITEM or parent::TD) and (contains(.,'&#xA;') or contains(.,'&#xD;')))">
                <pre>
                    <xsl:apply-templates/>
                </pre>
            </xsl:when>
    ...
    <xsl:template name="page-entree">
        ...
        <html>
            <head>
                <title>
                    <xsl:value-of select="ancestor-or-self::XPAGES[last()]/INFORMATIONS/TITRE"/>
                </title>
            ...
            <body class="page">
                ...
                <div class="zone_sommaire">
                    <div class="zone_outils">
                        <a href="pages_{ancestor-or-self::XPAGES[1]/INFORMATIONS/LABEL}/sommaire.html"
                           title="{$messages/message[@label='Sommaire']}">
                            <img src="{$interface}/sommaire_petit.png" width="25" height="25" border="0"
                                 alt="{$messages/message[@label='Sommaire']}"/>
                        </a>
                        <xsl:if test="//GLOSSAIRE">
                            &#xA0;&#xA0;
                            <a href="pages_{ancestor-or-self::XPAGES[1]/INFORMATIONS/LABEL}/glossaire.html"
                               title="{$messages/message[@label='Glossaire']}">
                                <img src="{$interface}/glossaire_petit.png" width="25" height="25" border="0"
                                     alt="{$messages/message[@label='Glossaire']}"/>
                            </a>
                        </xsl:if>
                    </div>
                    ...
                    <div class="zone_texte">
                        ...
                        <xsl:choose>
                            ...
                            <xsl:otherwise>
                                <p>
                                    <xsl:value-of
                                            select="$messages/message[@label='Vous pouvez définir une page d_accueil du site en créant une page avec le label _index_.']"/>
                                </p>
                            </xsl:otherwise>
                        </xsl:choose>
                    </div>
    ...
    <xsl:template name="page-sommaire">
        ...
        <html>
            <head>
                <title>
                    <xsl:value-of select="$messages/message[@label='Sommaire']"/>
                </title>
                <link type="text/css" rel="stylesheet" href="{$prefixe}{$interface}/style.css"/>
            </head>
            ...
            <body class="page">
                ...
                <div class="zone_sommaire">
                    <div class="zone_outils">
                        <a href="{$prefixe}index.html" title="{$messages/message[@label='Entrée du site']}">
                            <img src="{$prefixe}{$interface}/home.png" width="25" height="25" alt="home" border="0"/>
                        </a>
                        <xsl:if test="//GLOSSAIRE">
                            &#xA0;&#xA0;
                            <a href="glossaire.html" title="{$messages/message[@label='Glossaire']}">
                                <img src="{$prefixe}{$interface}/glossaire_petit.png" width="25" height="25" border="0"
                                     alt="{$messages/message[@label='Glossaire']}"/>
                            </a>
                        </xsl:if>
                    </div>
    ...
    <xsl:template match="XPAGES" mode="sommaire-site">
        ...
        <li>
            ...
            <a href="{$prefixe}pages_{@labelfichiers}/{$nom-fichier-index}" class="sommaire2">
                <xsl:value-of select="INFORMATIONS/TITRE"/>
            </a>
    ...
    <xsl:template name="page-glossaire">
        ...
        <html>
            <head>
                <title>
                    <xsl:value-of select="$messages/message[@label='Glossaire']"/>
                </title>
                <link type="text/css" rel="stylesheet" href="{$prefixe}{$interface}/style.css"/>
            </head>
            <body class="page">
                ...
                <div class="zone_sommaire">
                    <div class="zone_outils">
                        ...
                        &#xA0;&#xA0;
                        <a href="sommaire.html" title="{$messages/message[@label='Sommaire']}">
                            <img src="{$prefixe}{$interface}/sommaire_petit.png" width="25" height="25" border="0"
                                 alt="{$messages/message[@label='Sommaire']}"/>
                        </a>
                        <!--
                ...
                <div class="zone_contenu_glossaire">
                    <div class="zone_texte">
                        ...
                        <br/>
                        <br/>
                        <!-- pour que les liens pointent vers le haut de la page -->
                    </div>
                </div>
            </body>
        </html>
    ...
    <xsl:template name="lettres-glossaire">
        ...
        <xsl:variable name="mots"
                      select="//GLOSSAIRE[translate(substring(@ref,1,1),'aàbcçdeéèêfghiïîjklmnoôpqrstuùvwxyz','AABCCDEEEEFGHIIIJKLMNOOPQRSTUUVWXYZ')=$lettre or (not(@ref) and translate(substring(@mot,1,1),'aàbcçdeéèêfghiïîjklmnoôpqrstuùvwxyz','AABCCDEEEEFGHIIIJKLMNOOPQRSTUUVWXYZ')=$lettre)]"/>
        <xsl:if test="$mots">
            <dl>
        ...
        <xsl:if test="string-length($lettres)&gt;1">
            <xsl:call-template name="lettres-glossaire">
                <xsl:with-param name="lettres">
                    <xsl:value-of select="substring($lettres,2)"/>
                </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
    </xsl:template>
    ...
    <xsl:template name="format-fichier">
        ...
        <xsl:variable name="ext">
            <xsl:call-template name="extension-fichier">
                <xsl:with-param name="nom" select="$nom"/>
            </xsl:call-template>
        </xsl:variable>
        <xsl:variable name="extmin"
    ...
    <xsl:template name="nom-fichier">
        ...
        <xsl:choose>
            <xsl:when test="contains($chemin,$sep)">
                <xsl:call-template name="nom-fichier">
                    <xsl:with-param name="chemin" select="substring-after($chemin,$sep)"/>
                </xsl:call-template>
            </xsl:when>
            <xsl:otherwise>
    ...
    <xsl:template name="rep-uri">
        ...
        <xsl:choose>
            <xsl:when test="contains($chemin,$sepuri) and contains(substring-after($chemin,$sepuri),$sepuri)">
                <xsl:value-of select="substring-before($chemin,$sepuri)"/><xsl:value-of select="$sepuri"/>
                <xsl:call-template name="rep-uri">
                    <xsl:with-param name="chemin" select="substring-after($chemin,$sepuri)"/>
                </xsl:call-template>
            </xsl:when>
            <xsl:otherwise>
    ...
    <xsl:template name="rep-fichier">
        ...
        <xsl:choose>
            <xsl:when test="contains($chemin,$sep) and contains(substring-after($chemin,$sep),$sep)">
                <xsl:value-of select="substring-before($chemin,$sep)"/><xsl:value-of select="$sep"/>
                <xsl:call-template name="rep-fichier">
                    <xsl:with-param name="chemin" select="substring-after($chemin,$sep)"/>
                </xsl:call-template>
            </xsl:when>
    ...
    <xsl:template name="ecrire-fichier">
        ...
        <!-- chemin du fichier dans lequel on veut écrire -->
        <xsl:param name="contenu"/>
        <!-- contenu à écrire dans le fichier -->
        <xsl:param name="methode">
            <xsl:choose>
                <xsl:when test="translate(substring($fichier, string-length($fichier)-4, 4), 'XML', 'xml') = '.xml'">
                    xml
                </xsl:when>
                <xsl:otherwise>html</xsl:otherwise>
            </xsl:choose>
        </xsl:param>
        <xsl:param name="encodage" select="'UTF-8'"/>
        <!-- Sablotron ne connait pas autre chose -->
        ...
        <xsl:choose>
            ...
            <xsl:when test="element-available('exsl:document')">
                <exsl:document href="file://{$fichier}" method="{$methode}" encoding="{$encodage}"
                               indent="{$indentation}">
                    <xsl:copy-of select="$contenu"/>
                </exsl:document>
            </xsl:when>
            ...
            <xsl:otherwise>
                <xsl:message terminate="yes">
                    <xsl:value-of
                            select="$messages/message[@label='Impossible de créer un fichier avec le processeur XSLT de']"/>
        ...
        <xsl:call-template name="ecrire-fichier">
            ...
            <xsl:with-param name="contenu">
                ...
                <html>
                    <head>
                        <title>
                            <xsl:value-of select="$messages/message[@label='Liste des liens non résolus']"/>
                        </title>
                    </head>
                    <body>
                        <div style="text-align: center">
                        ...
                        <div style="text-align: center">
                            <h1>
                                <xsl:value-of select="$messages/message[@label='Mots du glossaire non définis']"/>
                            </h1>
                        </div>
                        <ul>
                        ...
                        <div style="text-align: center">
                            <h1>
                                <xsl:value-of
                                        select="$messages/message[@label='Mots du glossaire définis plus d_une fois']"/>
                            </h1>
                        </div>
                        <ul>
                        ...
                        <p>
                            <a href="definitions_glossaire.html">
                                <xsl:value-of
                                        select="$messages/message[@label='Localisation des définitions du glossaire']"/>
                            </a>
                        </p>
                    </body>
                </html>
        ...
        <xsl:choose>
            <xsl:when test="@contribution='' or not(@contribution)">
                <xsl:if test="count(ancestor::XPAGES[INFORMATIONS/LABEL=@labelfichiers][1]//PAGE[@label=$page]) != 1">
                    <li>
                        <a href="{concat('pages_',ancestor::XPAGES[1]/@labelfichiers)}/{ancestor::PAGE/@label}.html"
                           class="lien">
                            <xsl:value-of select="$messages/message[@label='page']"/>
                            <xsl:text> </xsl:text>
                            <xsl:value-of select="@page"/>
                            <xsl:text> </xsl:text><xsl:value-of
                                select="$messages/message[@label='de la même contribution']"/>
                        </a>
                        <br/>
                    </li>
                </xsl:if>
            ...
            <xsl:when test="@page='' or not(@page)">
                <xsl:if test="count(//XPAGES[INFORMATIONS/LABEL=$contribution]) != 1">
                    <li>
                        <a href="{concat('pages_',ancestor::XPAGES[1]/@labelfichiers)}/{ancestor::PAGE/@label}.html"
                           class="lien">
                            <xsl:value-of select="$messages/message[@label='contribution']"/>
                            <xsl:text> </xsl:text>
                        ...
                        <xsl:if test="count(//XPAGES[INFORMATIONS/LABEL=$contribution]) &gt; 1">
                            <xsl:text> </xsl:text>
                            <xsl:value-of
                                    select="$messages/message[@label='(il y a plus d_une contribution avec ce label)']"/>
                        </xsl:if>
                        <br/>
                    </li>
                </xsl:if>
            ...
            <xsl:otherwise>
                <xsl:if test="count(//XPAGES[INFORMATIONS/LABEL=$contribution]//PAGE[@label=$page]) != 1">
                    <li>
                        ...
                        <a href="{concat('pages_',ancestor::XPAGES[1]/@labelfichiers)}/{ancestor::PAGE/@label}.html"
                           class="lien">
                            <xsl:value-of select="concat($contribution, '/', $page)"/>
                        </a>
                        <br/>
                    </li>
                </xsl:if>
    ...
    <xsl:template match="LIENIMAGE" mode="mauvais-liens">
        ...
        <xsl:if test="count(//XPAGES[INFORMATIONS/LABEL=$contribution]/PAGE[@label=$page]//ENVIMAGE[@label=$label]) != 1">
            <li>
                ...
                <a href="{concat('pages_',$contribution,'/',$page,'.html')}" class="lien">
                    <xsl:value-of select="concat($contribution, '/', $page)"/>
                </a>
                <br/>
            </li>
        ...
        <xsl:if test="$motref != ''">
            <xsl:if test="normalize-space(.)=''">
                <xsl:if test="count(//GLOSSAIRE[(@ref=$motref or @mot=$motref) and normalize-space(.)!=''])=0">
                    <li>
                        <xsl:value-of select="$motref"/> (<xsl:value-of
                            select="$messages/message[@label='utilisé page']"/><xsl:text> </xsl:text><xsl:value-of
                            select="concat(ancestor::XPAGES[1]/INFORMATIONS/LABEL, '_', ancestor::PAGE[1]/@label)"/>)
                    </li>
                </xsl:if>
            </xsl:if>
        </xsl:if>
        ...
        <xsl:call-template name="ecrire-fichier">
            ...
            <xsl:with-param name="contenu">
                ...
                <html>
                    <head>
                        <title>
                            <xsl:value-of select="$messages/message[@label='Labels utilisés']"/>
                        </title>
                    </head>
                    <body>
                        <p>
                            <a href="#erreurs">
                                <xsl:value-of select="$messages/message[@label='Erreurs']"/>
                            </a>
                            <xsl:text> - </xsl:text>
                            <a href="#contributions">
                                <xsl:value-of select="$messages/message[@label='Contributions']"/>
                            </a>
                            <xsl:text> - </xsl:text>
                            <a href="#pages">
                                <xsl:value-of select="$messages/message[@label='Pages']"/>
                            </a>
                            <xsl:text> - </xsl:text>
                            <a href="#index">
                                <xsl:value-of select="$messages/message[@label='Index']"/>
                            </a>
                        </p>
                        ...
                        <xsl:for-each select="//XPAGES | //PAGE">
                            ...
                            <xsl:choose>
                                ...
                                <xsl:otherwise>
                                    <em>
                                        <xsl:value-of select="$messages/message[@label='sans label']"/>
                                    </em>
                                </xsl:otherwise>
                            </xsl:choose>
                            &#xA0;&#xA0;&#xA0;&#xA0;
                            <xsl:value-of select="$type"/> &quot;<xsl:value-of select="$titre"/>&quot;
                            <br/>
                        </xsl:for-each>
                    </body>
                </html>
    ...
    <xsl:template name="labels-xpages">
        <xsl:if test="XPAGES">
            <ul>
                <xsl:for-each select="XPAGES">
                    <li>
                        <a name="xpages{INFORMATIONS/LABEL}"/>XPAGES &quot;<xsl:value-of select="INFORMATIONS/TITRE"/>&quot;:
                        <tt>
                            <xsl:value-of select="INFORMATIONS/LABEL"/>
                        </tt>
                        <xsl:call-template name="labels-xpages"/>
                    </li>
                </xsl:for-each>
            ...
            <ul>
                <xsl:for-each select="*[self::XPAGES|self::PAGE]">
                    <li>
                        <xsl:choose>
                            <xsl:when test="self::XPAGES">
                                XPAGES &quot;<xsl:value-of select="INFORMATIONS/TITRE"/>&quot;:
                                <tt>
                                    <xsl:value-of select="INFORMATIONS/LABEL"/>
                                </tt>
                            </xsl:when>
                            <xsl:when test="self::PAGE">
                                <a name="page{@label}"/>PAGE &quot;<xsl:value-of select="@titre"/>&quot;:
                                <tt>
                                    <xsl:value-of select="@label"/>
                                </tt>
                            </xsl:when>
    ...
    <xsl:template name="labels-erreurs">
        ...
        <p>
            <xsl:value-of
                    select="$messages/message[@label='Un lien utilisant un label avec un doublon ne fonctionnera pas, et apparaîtra sur la page des']"/>
            <xsl:text> </xsl:text>
            <a href="mauvais_liens.html">
                <xsl:value-of select="$messages/message[@label='mauvais liens']"/>
            </a>
            .
        </p>
        <xsl:if test="//XPAGES[INFORMATIONS/LABEL=following::XPAGES/INFORMATIONS/LABEL]"> <!-- bug XSLTC XALANJ-2031 -->
            <h2>
                <xsl:value-of select="$messages/message[@label='Doublons d_ensembles XPAGES']"/>
            </h2>
            <ul>
                <xsl:for-each select="//XPAGES">
                    <xsl:if test="count(//XPAGES[INFORMATIONS/LABEL=current()/INFORMATIONS/LABEL])&gt;1">
                        <li>XPAGES &quot;<xsl:value-of select="INFORMATIONS/TITRE"/>&quot;:
                            <tt>
                                <xsl:value-of select="INFORMATIONS/LABEL"/>
                            </tt>
                            <xsl:text> - </xsl:text>
                            <a href="#xpages{INFORMATIONS/LABEL}">
                                <xsl:value-of select="$messages/message[@label='lien vers le premier avec ce label']"/>
                            </a>
                        </li>
                    </xsl:if>
                </xsl:for-each>
        ...
        <xsl:if test="//PAGE[@label=following::PAGE/@label]">
            <h2>
                <xsl:value-of select="$messages/message[@label='Doublons de pages']"/>
            </h2>
            <ul>
                <xsl:for-each select="//PAGE">
                    <xsl:if test="count(//PAGE[@label=current()/@label])&gt;1">
                        <li>
                            <xsl:value-of select="$messages/message[@label='page']"/> &quot;<xsl:value-of
                                select="@titre"/>&quot;:
                            <tt>
                                <xsl:value-of select="@label"/>
                            </tt>
                            -
                            <a href="#page{@label}">
                                <xsl:value-of select="$messages/message[@label='lien vers la première avec ce label']"/>
                            </a>
                        </li>
                    </xsl:if>
                </xsl:for-each>
        ...
        <xsl:if test="//XPAGES[normalize-space(translate(INFORMATIONS/LABEL,'abcdefghijklmnopqrstuvwxyz0123456789- ','                                     x'))!=''] or //PAGE[normalize-space(translate(@label,'abcdefghijklmnopqrstuvwxyz0123456789- ','                                     x'))!=''] or //ENVIMAGE[@label!='' and normalize-space(translate(@label,'abcdefghijklmnopqrstuvwxyz0123456789- ','                                     x'))!='']">
            <h2>
                <xsl:value-of select="$messages/message[@label='Labels invalides']"/>
            </h2>
            <ul>
                <xsl:for-each select="//XPAGES">
                    <xsl:if test="normalize-space(translate(INFORMATIONS/LABEL,'abcdefghijklmnopqrstuvwxyz0123456789- ','                                     x'))!=''">
                        <li>
                            <xsl:value-of select="name()"/> &quot;<xsl:value-of select="INFORMATIONS/TITRE"/>&quot;:
                            <tt>
                                <xsl:value-of select="INFORMATIONS/LABEL"/>
                            </tt>
                        </li>
                ...
                <xsl:for-each select="//PAGE|//ENVIMAGE[@label!='']">
                    <xsl:if test="normalize-space(translate(@label,'abcdefghijklmnopqrstuvwxyz0123456789- ','                                     x'))!=''">
                        <li>
                            <xsl:value-of select="name()"/> &quot;<xsl:value-of select="@titre"/>&quot;<xsl:text> </xsl:text><xsl:value-of
                                select="$messages/message[@label='contribution']"/><xsl:text> </xsl:text>
                            <tt>
                                <xsl:value-of select="ancestor::XPAGES[1]/@labelfichiers"/>
                            </tt>
                            :
                            <tt>
                                <xsl:value-of select="@label"/>
        ...
        <xsl:call-template name="ecrire-fichier">
            ...
            <xsl:with-param name="contenu">
                ...
                <html>
                    <head>
                        <title>
                            <xsl:value-of select="$messages/message[@label='Commentaires des auteurs']"/>
                        </title>
                    </head>
                    <body>
                        <h1>
                        ...
                        <ul>
                            <xsl:for-each select="//COMMENTAIRE">
                                <li>
                                    ...
                                    -
                                    <xsl:value-of select="$messages/message[@label='page']"/><xsl:text> </xsl:text>
                                    <a href="pages_{concat(ancestor::XPAGES[1]/@labelfichiers, '/', ancestor::PAGE[1]/@label, '.html')}">
                                        <xsl:value-of select="ancestor::PAGE/@titre"/>
                                    </a>
                                </li>
                            </xsl:for-each>
                        </ul>
    ...
    <xsl:template name="credits">
        <xsl:variable name="interface">interface/
            <xsl:choose>
                <xsl:when test="/XPAGES/INFORMATIONS/INTERFACE">
                    <xsl:value-of select="/XPAGES/INFORMATIONS/INTERFACE"/>
                </xsl:when>
                <xsl:otherwise>
                    <xsl:value-of select="$interface_par_defaut"/>
                </xsl:otherwise>
            ...
            <xsl:with-param name="contenu">
                ...
                <html>
                    <head>
                        <title>
                            <xsl:value-of
                                    select="$messages/message[@label='Liste des images et des crédits, triée par crédit']"/>
                        </title>
                    ...
                    <body>
                        ...
                        <table class="tableau">
                            <tr>
                                ...
                                <th>
                                    <xsl:value-of select="$messages/message[@label='Image']"/>
                                </th>
                                <th>
                                    <xsl:value-of select="$messages/message[@label='Titre']"/>
                                </th>
                                <th>
                                    <xsl:value-of select="$messages/message[@label='Nom fichier']"/>
                                </th>
                                <th>
                                    <xsl:value-of select="$messages/message[@label='Légende']"/>
                                </th>
                                <th>
                                    <xsl:value-of select="$messages/message[@label='page']"/>
                                </th>
    ...
    <xsl:template match="ENVIMAGE" mode="credits">
        <tr>
            ...
            <xsl:variable name="nomf">
                <xsl:choose>
                    <xsl:when test="$format='MPEG' or $format='MPEG4' or $format='OGG' or $format='WEBM'">animation
                    </xsl:when>
                    <xsl:otherwise>
                        <xsl:value-of select=".//FICHIER[1]/@nom"/>
                    </xsl:otherwise>
                </xsl:choose>
            </xsl:variable>
            <xsl:variable name="url"
                          select="concat('pages_', ancestor::XPAGES[1]/@labelfichiers, '/', ancestor::PAGE[1]/@label, '.html')"/>
            <td>
            ...
            <td>
                <a href="{$url}">
                    <xsl:choose>
                        <xsl:when test="$nomf='animation'">
                            <xsl:value-of select="$messages/message[@label='animation']"/>
                        </xsl:when>
                        <xsl:otherwise>
                            <img src="{concat('pages_', ancestor::XPAGES[1]/@labelfichiers, '/', $nomf)}" alt="{$nomf}"
                                 width="200"/>
                        </xsl:otherwise>
                    </xsl:choose>
                </a>
            ...
            <td>
                <a href="{$url}">
                    <xsl:value-of select="ancestor::PAGE[1]/@titre"/>
                </a>
            </td>
        </tr>
    </xsl:template>
                        ...
                        <ul>
                            <xsl:for-each select="//GLOSSAIRE[.!='']">
                                ...
                                <li>
                                    ...
                                    :
                                    <xsl:value-of select="$messages/message[@label='contribution']"/><xsl:text> </xsl:text>
                                    <tt>
                                        <xsl:value-of select="ancestor::XPAGES[1]/@labelfichiers"/>
                                    </tt>
                                    ,
                                    <xsl:if test="ancestor::XPAGES[1]/INFORMATIONS/LABEL != ancestor::XPAGES[1]/@labelfichiers">
                                        XPAGES
                                        <tt>
                                            <xsl:value-of select="ancestor::XPAGES[1]/INFORMATIONS/LABEL"/>
                                        </tt>
                                        ,
                                    </xsl:if>
                                    PAGE
                                    <tt>
                                        <xsl:value-of select="ancestor::PAGE[1]/@label"/>
                                    </tt>
                                </li>
                            </xsl:for-each>
                        </ul>
        ...
        <xsl:call-template name="ecrire-fichier">
            ...
            <xsl:with-param name="contenu">
                ...
                <html>
                    <head>
                        <title>
                            <xsl:value-of select="$messages/message[@label='Liste des exercices']"/>
                        </title>
                        <link type="text/css" rel="stylesheet" href="{$interface}/{$feuille-de-style}"/>
                    </head>
                    <body>
                        ...
                        <table class="tableau">
                            <xsl:for-each select="//XPAGES[.//QCM | .//EXERCICE]">
                                <xsl:if test="count(ancestor::XPAGES) &gt; 0">
                                    <tr>
                                        <td>
                                            <xsl:element name="h{count(ancestor::XPAGES)+2}">
                                                <xsl:value-of select="INFORMATIONS/TITRE"/>
                                            </xsl:element>
                                        </td>
                                        <td></td>
                                    ...
                                    <xsl:for-each select=".//PAGE[.//EXERCICE|.//QCM]">
                                        <tr>
                                            <xsl:variable name="urlpage"
                                                          select="concat('pages_', ancestor::XPAGES[1]/@labelfichiers, '/', @label, '.html')"/>
                                            <td>
                                                <a href="{$urlpage}">
                                                    <xsl:value-of select="@titre"/>
                                                </a>
                                                ...
                                                <td>
                                                    <xsl:choose>
                                                        <xsl:when test="self::EXERCICE">
                                                            ...
                                                            <xsl:value-of
                                                                    select="$messages/message[@label=concat('Exercice d_', $type)]"/>
                                                        </xsl:when>
                                                        <xsl:when test="self::QCM">
                                                            <xsl:value-of select="$messages/message[@label='QCM']"/>
                                                        </xsl:when>
                                                    </xsl:choose>
                                                </td>
